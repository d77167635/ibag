...
const money = (value: unknown) => typeof value === "number" && Number.isFinite(value) ? `${value < 0 ? "−" : ""}$${Math.abs(value).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : "—";
...
const layerBindings: Record<string,string> = {
  state:"financial_state", causal:"causal_analysis", decisions:"decision_intelligence", consequence:"consequence_model",
  optimization:"optimization_intelligence", goals:"goal_intelligence", graph:"evidence_graph", uncertainty:"uncertainty",
  reasoning:"reasoning", temporal:"temporal", behavior:"category_drift",
};
...
export function IrisIntelligenceScreens({ page, intel, go }: ScreenProps) {
  ...
  const rows=useMemo<Row[]>(()=>{
    if(page==="iris/findings") return [...(reasoning.risks??[]).map((x:any)=>({label:"Risk",value:x.statement??x,state:x.evidence??"calculated",detail:x.evidence??"Reasoning-derived finding"})),...(reasoning.opportunities??[]).map((x:any)=>({label:"Opportunity",value:x.statement??x,state:x.evidence??"calculated",detail:"Reasoning-derived opportunity"})),...(reasoning.unresolvedQuestions??reasoning.unknowns??[]).map((x:any)=>({label:"Unknown",value:x,state:"insufficient_evidence",detail:"Evidence gap"}))];
    ...
    if(page==="iris/decisions") return (decisions.options??[]).map((x:any)=>({label:x.label??"Decision option",value:x.rationale??x,state:x.evidence_state??"calculated",detail:`Readiness: ${x.readiness??decisions.quality??"analysis only"}`}));
    ...
    if(page==="iris/goals") return [...(goals.goals??goals.activeGoals??[]).map((x:any)=>({label:x.title??x.objective??"Goal",value:x,state:x.evidence_state??"calculated",detail:x.source??"Goal intelligence"})),...(goals.conflicts??[]).map((x:any)=>({label:"Goal conflict",value:x,state:"calculated",detail:"Tradeoff detected"}))];
    ...
    if(page==="iris/behavior") return Array.isArray(behavior)?behavior.map((x:any)=>({label:x.subdomainLabel??x.category??"Category",value:x,state:x.evidence??x.state??"calculated",detail:`Baseline transactions: ${x.baselineTransactionCount??"—"}`})):Object.entries(behavior??{}).map(([k,v])=>({label:k,value:v,state:"calculated",detail:"Behavior intelligence"}));
    return [];
