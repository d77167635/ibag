...
export function PlaidDashboard({ surface, selection, onRefresh }: { surface: Surface | null; selection: Selection | null; onRefresh: () => void }) {
  ...
  return <section className="plaid-page">
    ...
    <div className="plaid-rule"><strong>Source boundary</strong><span>Plaid products → provider information only</span><span className="plaid-separator">•</span><span>Iris interpretation → Iris Features</span></div>
    ...
    <div className="plaid-section-title"><div><span>PRODUCT INVENTORY</span><h2>Products available to this connected data set</h2></div><small>{products.length} product domains observed from Item state</small></div>
    <div className="plaid-product-grid">{products.map((product) => <article className="plaid-product" key={product.product}><div className="plaid-product-head"><span>{title(product.product)}</span><b className={`plaid-product-status ${product.status}`}>{title(product.status ?? "unknown")}</b></div><p>Provider product domain. Information shown here is limited to the product's own observed/provider state.</p>{product.capabilities?.length ? <div className="plaid-capabilities"><span>Used by Iris selection engine</span><p>{product.capabilities.map(title).join(" · ")}</p></div> : null}</article>)}</div>
    {selection && <section className="plaid-selection"><div><span className="plaid-kicker">IRIS PRODUCT SELECTION</span><h2>Which Plaid capabilities Iris values most</h2><p>Iris ranks only products that are actually present, consented, or available for a connected Item. This is a selection decision, not automatic consent or activation.</p></div><div className="plaid-ranking">{selection.selected.slice(0, 10).map((product, index) => <div className="plaid-rank" key={product.product}><strong>{String(index + 1).padStart(2, "0")}</strong><span>{title(product.product)}</span><b>{product.score ?? "—"}</b><small>{product.capabilities?.map(title).join(" · ")}</small></div>)}</div></section>}
  </section>;
}
