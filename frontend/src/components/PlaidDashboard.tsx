...
export function PlaidDashboard({ surface, onRefresh }: { surface: Surface | null; selection?: unknown; onRefresh: () => void }) {
  ...
  function Transactions() { return <div className="plaid-table-wrap"><table className="plaid-table"><thead><tr><th>Observed</th><th>Account</th><th>Provider transaction</th><th>Evidence</th></tr></thead><tbody>{evidenceFor("transactions").map((row) => <tr key={row.id}><td>{row.effective_at ? new Date(row.effective_at).toLocaleDateString() : row.fetched_at ? new Date(row.fetched_at).toLocaleDateString() : "—"}</td><td>{row.account_id ?? "—"}</td><td>{row.provider_object_id ?? "—"}</td><td>{row.evidence_state ?? "observed"}</td></tr>)}</tbody></table>{!evidenceFor("transactions").length && <div className="plaid-empty">No current observed Transactions evidence.</div>}</div>; }
