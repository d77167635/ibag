...
type Product = { key?: string; product?: string; displayName?: string; status?: string; category?: string; description?: string; phase1Relevant?: boolean; irisCapabilities?: string[]; capabilities?: string[]; item_count?: number; active_item_count?: number; consented_item_count?: number; available_item_count?: number; unavailable_item_count?: number; score?: number };
type Item = { institution_name: string | null; status: string; last_synced_at: string | null; selected?: Product[]; billed_products?: string[]; available_products?: string[]; consented_products?: string[] };
...
export function PlaidDashboard({ surface, selection, onRefresh }: { surface: Surface | null; selection: Selection | null; onRefresh: () => void }) {
  ...
  const observedCount = products.filter((p) => p.status === "active" || p.status === "consented").length;

  return <section className="plaid-page">
    <div className="plaid-intro">
      <div><span className="plaid-kicker">PLAID SOURCE • READ ONLY</span><h1>Plaid Dashboard</h1><p>Every Plaid product domain in iBag is presented as provider/source information. Users can inspect it, ask Iris about it, and connect institutions, but cannot edit or rewrite Plaid-sourced facts.</p></div>
    ...
    <div className="plaid-source-summary">
      <div><span>CATALOG</span><strong>{products.length}</strong><small>provider product domains exposed</small></div>
      ...
      <div><span>OBSERVED / CONSENTED</span><strong>{observedCount}</strong><small>current connected-Item state</small></div>
    </div>
    ...
    <div className="plaid-institutions">{(surface?.items ?? []).map((item, index) => <article className="plaid-institution" key={`${item.institution_name ?? "institution"}-${index}`}><div><span className="plaid-status-dot"/><strong>{item.institution_name ?? "Connected institution"}</strong></div><span>{title(item.status)}</span><small>{item.last_synced_at ? `Last synced ${new Date(item.last_synced_at).toLocaleString()}` : "No sync timestamp"}</small><details><summary>View provider product state</summary><div className="plaid-raw-list"><b>Active / billed</b><span>{item.billed_products?.length ? item.billed_products.map(title).join(" · ") : "None reported"}</span><b>Consented</b><span>{item.consented_products?.length ? item.consented_products.map(title).join(" · ") : "None reported"}</span><b>Available</b><span>{item.available_products?.length ? item.available_products.map(title).join(" · ") : "None reported"}</span></div></details></article>)}</div>

    <div className="plaid-section-title"><div><span>PLAID PRODUCT CATALOG</span><h2>Complete provider surface</h2></div><small>{surface?.catalog_version ?? "Current runtime catalog"}</small></div>
