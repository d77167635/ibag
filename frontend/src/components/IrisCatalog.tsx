  ...
  return (
    <div className="iis-screen">
      ...
      <div className="iis-metric-grid">
        <div className="iis-metric"><span>Active reports</span><strong>{active.length}</strong><small>{saving ? "Saving…" : message || "Reports currently activated for publication"}</small></div>
        <div className="iis-metric"><span>Report products</span><strong>{reportProductTotal}</strong><small>{metadata?.catalog_version ?? "Governed Iris report catalog"}</small></div>
        <div className="iis-metric"><span>Provider products</span><strong>Not connected</strong><small>Provider-product state is intentionally separate from Iris reports</small></div>
        <div className="iis-metric"><span>Report families</span><strong>{familyTotal}</strong><small>Families in the Iris report catalog</small></div>
      </div>

      <section className="iis-panel">
        <header><div><span>YOUR ACTIVE REPORTS</span><h2>Active reports</h2></div><button type="button" onClick={() => void reset()} disabled={saving}>Restore available reports</button></header>
        ...
        <div className="iis-catalog-grid">
          {catalog.filter((r) => active.includes(r.reportId)).map((r) => <button type="button" key={r.reportId} className="iis-catalog-card selected" onClick={() => setSelectedId(r.reportId)}><div><span>{r.family} · {r.outputType}</span><b>{r.name}</b></div><small>{r.description}</small><em>Active · Open report</em></button>)}
        </div>
      ...
      <section className="iis-panel">
        ...
        <div className="iis-catalog-grid">
          {visible.map((r) => { const isActive = active.includes(r.reportId); return <button type="button" key={r.reportId} className={`iis-catalog-card${isActive ? " selected" : ""}`} onClick={() => setSelectedId(r.reportId)}><div><span>{r.family} · {r.outputType}</span><b>{r.name}</b></div><small>{r.description}</small><em>{isActive ? "Active · Open report" : "Open report · Activate inside"}</em></button>; })}
          {visible.length === 0 && <p className="iis-note">No report products match the current search or family filter.</p>}
        </div>
      </section>

      <section className="iis-panel">
        <header><div><span>PRODUCT BOUNDARY</span><h2>How Iris report products are created</h2></div></header>
        <div className="iis-boundary"><p><strong>Intelligence creates the products.</strong> Iris can combine observations, canonical facts, relationships, statistics, baselines, patterns, risk, opportunity, scenarios, decisions, consequences, outcomes, learning, and higher-order reasoning recursively. The resulting evidence-qualified reports and analytics are the user products.</p><p className="iis-note"><strong>Report catalog:</strong> {metadata?.product_boundary ?? "Report catalog metadata is separate from financial evidence."}</p><p className="iis-note"><strong>Provider boundary:</strong> {metadata?.provider_boundary ?? "Provider products and observations are separate from report activation and catalog metadata."}</p></div>
      </section>
    </div>
  );
}
