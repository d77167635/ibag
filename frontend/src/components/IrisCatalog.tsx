...
export function IrisCatalog({ go }: { go?: (page: string) => void }) {
  ...
  const visible = useMemo(() => catalog.filter((r) => (family === "all" || r.family === family) && (!query.trim() || `${r.name} ${r.description} ${r.family} ${r.outputType}`.toLowerCase().includes(query.toLowerCase()))), [catalog, family, query]);
  ...
  return <div className="iis-screen">
    ...
    <section className="iis-panel">
      ...
      <div className="iis-catalog-grid">{catalog.filter((r) => active.includes(r.reportId)).map((r) => <button type="button" key={r.reportId} className="iis-catalog-card selected" onClick={() => setSelectedId(r.reportId)}><div><span>{r.family} · {r.outputType}</span><b>{r.name}</b></div><small>{r.description}</small><em>Active · Open product</em></button>)}</div>
    ...
    <section className="iis-panel">
      ...
      <div className="iis-catalog-grid">{visible.map((r) => { const isActive = active.includes(r.reportId); return <button type="button" key={r.reportId} className={`iis-catalog-card${isActive ? " selected" : ""}`} onClick={() => setSelectedId(r.reportId)}><div><span>{r.family} · {r.outputType}</span><b>{r.name}</b></div><small>{r.description}</small><em>{isActive ? "Active · Open product" : "Open product · Activate inside"}</em></button>; })}</div>
    </section>
