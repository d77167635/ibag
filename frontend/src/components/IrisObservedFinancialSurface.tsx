...
export function IrisObservedFinancialSurface({ overview, intel }: Props) {
  ...
  return <section style={panel} aria-label="Iris observed financial surface">
    ...
    <div className="iris4-metrics" style={{ marginTop: 12 }}>
      ...
      <div style={card}><span>Transactions observed</span><strong>{tx.length}</strong><small>Most recent provider-derived records in the dashboard window</small></div>
    ...
    <div style={{ ...card, marginTop: 12 }}><div className="iris4-panel-head"><span>OBSERVATION BOUNDARY</span><Evidence state={intel?.intelligence_gate?.status === "ready" ? "calculated" : "limited"}/></div><p className="iris4-note">Transactions shown here are provider-derived records available to the dashboard. Merchant and category totals are calculations over those displayed records, not provider assertions. Iris higher-order conclusions remain evidence-gated.</p></div>
  </section>;
}
