...
import { IrisCommandSurfaceSafe } from "./components/IrisCommandSurfaceSafe";
...
export default function App(){const[session,setSession]=useState<Session|null>(null),[checkedAuth,setCheckedAuth]=useState(false),[recovery,setRecovery]=useState(isRecoveryUrl()),initial=readWorkspace(),[workspace,setWorkspace]=useState(initial.workspace),[irisPage,setIrisPage]=useState(initial.irisPage),[signingOut,setSigningOut]=useState(false);
 ...
 if(!checkedAuth)return <main className="iris4-screen"><div className="iris4-empty"><span>IRIS</span><strong>Checking your session…</strong><p>Authenticating securely.</p></div></main>;if(recovery&&session)return <Auth recovery onRecoveryComplete={()=>setRecovery(false)}/>;if(!session)return <Auth/>;
 ...
 if(workspace==="plaid")return <div className="app-workspace app-workspace-plaid"><PlaidCommandSurface/>{switcher}{account}</div>;
 return <div className="app-workspace app-workspace-iris"><IrisCommandSurfaceSafe page={irisPage} go={(p)=>navigate("iris",p)}/>{switcher}{account}</div>;
}
