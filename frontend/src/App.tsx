...
const accountControlStyle: React.CSSProperties={position:"fixed",right:20,bottom:18,zIndex:2147483647,pointerEvents:"auto",display:"flex",alignItems:"center",gap:9,padding:"7px 9px 7px 11px",border:"1px solid rgba(255,255,255,.11)",borderRadius:10,background:"rgba(7,9,14,.98)",boxShadow:"0 10px 30px rgba(0,0,0,.5)"};
...
const signOutStyle:React.CSSProperties={border:"1px solid rgba(255,255,255,.16)",borderRadius:7,padding:"7px 10px",background:"rgba(255,255,255,.055)",color:"#fff",font:"800 8px/1 Inter,system-ui,sans-serif",letterSpacing:".06em",cursor:"pointer",pointerEvents:"auto"};
...
export default function App(){const[session,setSession]=useState<Session|null>(null),[checkedAuth,setCheckedAuth]=useState(false),[recovery,setRecovery]=useState(isRecoveryUrl()),initial=readWorkspace(),[workspace,setWorkspace]=useState(initial.workspace),[irisPage,setIrisPage]=useState(initial.irisPage),[signingOut,setSigningOut]=useState(false);
 ...
 const accountPortal=typeof document!=="undefined"?createPortal(account,document.body):account;
 ...
 if(workspace==="plaid")return <div className="app-workspace app-workspace-plaid"><PlaidDeepSurface/>{switcher}{accountPortal}<IrisAssistant/></div>;
 return <div className="app-workspace app-workspace-iris"><IrisCommandSurface page={irisPage} go={(p)=>navigate("iris",p)}/>{irisPage==="iris/source"&&<IrisSourceDataExplorer/>}{switcher}{accountPortal}<IrisAssistant/></div>;
}