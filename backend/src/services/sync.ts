...
export async function fullSyncForItem(itemDbId: string, userId: string, accessToken: string, idempotencyKey?: string) {
  ...
  const { data: run, error: runError } = await supabaseAdmin.rpc("begin_sync_run", {
    p_user_id: userId,
    p_item_id: itemDbId,
    p_idempotency_key: key,
  });
  if (runError || !run?.[0]) throw runError ?? new Error("Unable to create sync run");

  const runId = run[0].id as string;
  const existingState = run[0].state as string;
  if (existingState === "completed" || existingState === "validated") return;
  ...
  try {
    ...
    for (const acct of accountsResp.data.accounts) {
      const { data: row, error } = await supabaseAdmin.from("plaid_accounts").upsert({
        user_id: userId,
        item_id: itemDbId,
    ...
    let cursor: string | undefined = existingState === "retryable" ? (run[0].cursor as string | undefined) ?? undefined : undefined;
    let hasMore = true;
    let pagesProcessed = Number(run[0].pages_processed ?? 0);
    while (hasMore) {
      ...
      for (const tx of txResp.data.added ?? []) {
        ...
        const { data: rawRow, error: rawErr } = await supabaseAdmin.rpc("record_plaid_transaction_observation", {
          p_user_id: userId, p_account_id: localAccountId, p_plaid_transaction_id: tx.transaction_id, p_raw_response: tx,
        });
        ...
        const { error: normalizedError } = await supabaseAdmin.from("transactions").upsert({
          user_id: userId, account_id: localAccountId, raw_transaction_id: rawRow.id, plaid_transaction_id: tx.transaction_id,
      ...
      for (const removedTx of txResp.data.removed ?? []) {
        ...
        const { error: retireError } = await supabaseAdmin.rpc("retire_plaid_transaction_observation", { p_user_id: userId, p_plaid_transaction_id: removedTx.transaction_id });
        if (retireError) throw retireError;
        const { error: normalizedRetireError } = await supabaseAdmin.rpc("retire_normalized_transaction", { p_user_id: userId, p_plaid_transaction_id: removedTx.transaction_id, p_reason: "provider_removed" });
      ...
      pagesProcessed += 1;
      await updateSyncRun(runId, { state: "committing", cursor, pages_processed: pagesProcessed, added_count: addedCount, modified_count: modifiedCount, removed_count: removedCount, last_checkpoint_at: new Date().toISOString() });
    }

    await updateSyncRun(runId, { state: "reconciling" });
    await syncLiabilitiesForItem(userId, accessToken);
    for (const localAccountId of accountIdMap.values()) await recomputeRoundupsForAccount(userId, localAccountId, accessToken);
    await detectRecurringSeries(userId);
    await updateSyncRun(runId, { state: "intelligence_refresh" });
    const { error: itemError } = await supabaseAdmin.from("plaid_items").update({ last_synced_at: new Date().toISOString(), status: "active" }).eq("id", itemDbId).eq("user_id", userId);
    if (itemError) throw itemError;
    await updateSyncRun(runId, { state: "validated", completed_at: new Date().toISOString(), last_checkpoint_at: new Date().toISOString(), added_count: addedCount, modified_count: modifiedCount, removed_count: removedCount });
    await updateSyncRun(runId, { state: "completed" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown sync failure";
    try { await updateSyncRun(runId, { state: "retryable", error_message: message, completed_at: new Date().toISOString() }); }
    catch (checkpointError) { console.error("Unable to checkpoint failed sync:", checkpointError); }
    throw error;
  }
}
