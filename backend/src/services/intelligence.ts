...
export async function detectRecurringSeries(userId: string) {
  ...
  for (const tx of txs ?? []) { const list = byMerchant.get(tx.merchant_id) ?? []; list.push({ amount: Number(tx.amount), date: tx.posted_date, category: tx.plaid_category_detailed }); byMerchant.set(tx.merchant_id, list); }
  const median = (nums: number[]) => { const sorted = [...nums].sort((a, b) => a - b); const mid = Math.floor(sorted.length / 2); return sorted.length % 2 ? sorted[mid] : (sorted[mid - 1] + sorted[mid]) / 2; };
  ...
  for (const [merchantId, occurrences] of byMerchant) {
    ...
    const amounts = occurrences.map((o) => o.amount); const medianAmount = median(amounts); if (medianAmount <= 0) continue;
...
export async function computeRoundupProjection(userId: string, projectDays = 30) {
  const { data: txs } = await supabaseAdmin.from("transactions").select("amount, posted_date").eq("user_id", userId).eq("pending", false).gt("amount", 0);
  if (!txs || txs.length === 0) return { dailyRate: null, projected: null, basisDays: 0 };
  const totalRoundup = txs.reduce((sum, tx) => { const amount = Number(tx.amount); return sum + (Math.ceil(amount) - amount); }, 0);
  const dates = txs.map((tx) => new Date(tx.posted_date).getTime()); const spanDays = Math.max(1, Math.round((Math.max(...dates) - Math.min(...dates)) / 86_400_000)); const dailyRate = totalRoundup / spanDays;
...
export async function computeCashFlow(userId: string, windowDays = 30) {
  ...
  const inflow = sum(current, "in"); const outflow = sum(current, "out"); const net = inflow - outflow; const priorNet = sum(prior, "in") - sum(prior, "out"); const netChangePct = prior.length > 0 && priorNet !== 0 ? ((net - priorNet) / Math.abs(priorNet)) * 100 : null;
...
export async function computeForwardProjection(userId: string, days = 30) {
  const { data: checkingAccounts } = await supabaseAdmin.from("plaid_accounts").select("available_balance").eq("user_id", userId).eq("type", "depository").eq("subtype", "checking").not("available_balance", "is", null);
  ...
  const { data: series } = await supabaseAdmin.from("recurring_series").select("typical_amount, next_expected_date, interval_days, merchants(canonical_name)").eq("user_id", userId).eq("is_essential", true);
  const projected: { date: string; balance: number; event: string | null }[] = []; let balance = startBalance;
  for (let i = 0; i <= days; i++) { const d = new Date(Date.now() + i * 86_400_000).toISOString().slice(0, 10); const dueToday = (series ?? []).filter((s: any) => s.next_expected_date === d); let event: string | null = null; for (const bill of dueToday) { balance -= Number(bill.typical_amount); event = event ? `${event}, ${(bill as any).merchants?.canonical_name}` : (bill as any).merchants?.canonical_name; } projected.push({ date: d, balance, event }); }
...
export function buildNarrative(inputs: { safeToSpend: number | null; essentialBillsCount: number; cashFlowNet: number | null; cashFlowNetChangePct: number | null; debtChangePct: number | null; anomalyCount: number; }): string {
  ...
  if (inputs.cashFlowNet !== null) { parts.push(`Cash flow over the last 30 days is ${inputs.cashFlowNet >= 0 ? "positive" : "negative"} (${money(inputs.cashFlowNet)}).`); if (inputs.cashFlowNetChangePct !== null) parts.push(Math.abs(inputs.cashFlowNetChangePct).toFixed(0) === "0" ? "That's about the same as the prior 30 days." : `That's ${inputs.cashFlowNetChangePct >= 0 ? "up" : "down"} ${Math.abs(inputs.cashFlowNetChangePct).toFixed(0)}% versus the prior 30 days.`); }
  if (inputs.debtChangePct !== null) parts.push(`Revolving debt has ${inputs.debtChangePct >= 0 ? "increased" : "decreased"} ${Math.abs(inputs.debtChangePct).toFixed(0)}% over the last 30 days.`);
  if (inputs.anomalyCount > 0) parts.push(`${inputs.anomalyCount} transaction${inputs.anomalyCount === 1 ? " looks" : "s look"} unusually large compared to that merchant's typical amount — see below.`);
  return parts.join(" ");
}

export async function computeSpendingHierarchy(userId: string, windowDays = 30) {
  const windowStart = new Date(Date.now() - windowDays * 86_400_000).toISOString().slice(0, 10);
  const { data: txs } = await supabaseAdmin.from("transactions").select("amount, subdomains(label, domains(key, label))").eq("user_id", userId).eq("pending", false).gt("amount", 0).gte("posted_date", windowStart);
  if (!txs || txs.length === 0) return [];
  type DomainAcc = { key: string; label: string; total: number; subdomains: Map<string, number> }; const byDomain = new Map<string, DomainAcc>();
  for (const tx of txs as any[]) { const domain = tx.subdomains?.domains; const domainKey = domain?.key ?? "uncategorized"; const domainLabel = domain?.label ?? "Uncategorized"; const subdomainLabel = tx.subdomains?.label ?? "Uncategorized"; const entry = byDomain.get(domainKey) ?? { key: domainKey, label: domainLabel, total: 0, subdomains: new Map() }; entry.total += Number(tx.amount); entry.subdomains.set(subdomainLabel, (entry.subdomains.get(subdomainLabel) ?? 0) + Number(tx.amount)); byDomain.set(domainKey, entry); }
  const grandTotal = Array.from(byDomain.values()).reduce((s, d) => s + d.total, 0);
  return Array.from(byDomain.values()).map((d) => ({ key: d.key, label: d.label, amount: d.total, pctOfTotal: grandTotal > 0 ? (d.total / grandTotal) * 100 : 0, subdomains: Array.from(d.subdomains.entries()).map(([label, amount]) => ({ label, amount })).sort((a, b) => b.amount - a.amount) })).sort((a, b) => b.amount - a.amount);
}

export async function computeScenario(userId: string, type: "spending_change" | "bill_change" | "income_change", amount: number) {
  const [cashFlowSafety, cashFlow] = await Promise.all([computeCashFlowSafety(userId), computeCashFlow(userId)]);
  if (cashFlowSafety.safeToSpend === null || cashFlow.net === null) return { evidence: "insufficient_evidence" as const, reason: "Not enough account/transaction data to run a scenario yet." };
  let scenarioSafeToSpend = cashFlowSafety.safeToSpend; let scenarioNet = cashFlow.net; let assumption = "";
  if (type === "spending_change") { scenarioSafeToSpend -= amount; scenarioNet -= amount; assumption = `Assumes discretionary spending changes by exactly $${amount.toFixed(2)} with everything else held constant — a direct dollar-for-dollar substitution, not a behavioral prediction.`; }
  else if (type === "bill_change") { scenarioSafeToSpend -= amount; assumption = `Assumes essential bills change by $${amount.toFixed(2)} within the current 14-day horizon, with no other change.`; }
  else { if (cashFlow.inflow === null) return { evidence: "insufficient_evidence" as const, reason: "No inflow data to apply an income change to." }; const inflowDelta = cashFlow.inflow * (amount / 100); scenarioNet += inflowDelta; scenarioSafeToSpend += inflowDelta; assumption = `Assumes ${amount >= 0 ? "an increase" : "a decrease"} of ${Math.abs(amount).toFixed(0)}% applied to the observed 30-day inflow ($${cashFlow.inflow.toFixed(2)}), not a verified income figure — Iris does not yet distinguish income from other inflows.`; }
  return { evidence: "calculated" as const, baseline: { safeToSpend: cashFlowSafety.safeToSpend, cashFlowNet: cashFlow.net }, scenario: { safeToSpend: scenarioSafeToSpend, cashFlowNet: scenarioNet }, delta: { safeToSpend: scenarioSafeToSpend - cashFlowSafety.safeToSpend, cashFlowNet: scenarioNet - cashFlow.net }, assumption };
}
