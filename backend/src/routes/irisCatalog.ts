...
irisCatalogRouter.get("/iris/catalog", requireAuth, async (req: AuthedRequest, res) => {
  try {
    const { data, error } = await supabaseAdmin
      ...
      .select("catalog_version, selected_capability_ids, standard_name, updated_at")
      .eq("user_id", req.userId!)
      .maybeSingle();
    if (error) throw error;
    const selected = Array.isArray(data?.selected_capability_ids)
      ? data.selected_capability_ids.filter((id: unknown): id is string => typeof id === "string" && CATALOG.some(c => c.id === id))
    ...
    res.json({
      ...
      standard: { name: "Iris Standard", count: IRIS_STANDARD_CAPABILITY_IDS.length, capability_ids: IRIS_STANDARD_CAPABILITY_IDS },
      catalog: CATALOG,
...
irisCatalogRouter.put("/iris/catalog/selection", requireAuth, async (req: AuthedRequest, res) => {
  try {
    const ids = Array.isArray(req.body?.capability_ids)
      ? [...new Set(req.body.capability_ids.filter((v: unknown): v is string => typeof v === "string"))]
      : [];
    ...
    const { error } = await supabaseAdmin
      .from("iris_user_intelligence_preferences")
      .upsert({ user_id: req.userId!, catalog_version: IRIS_CATALOG_VERSION, selected_capability_ids: ids, standard_name: "Custom Iris Standard", updated_at: new Date().toISOString() }, { onConflict: "user_id" });
    if (error) throw error;
