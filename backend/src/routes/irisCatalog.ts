...
const selectionLimit = CATALOG.length;
...
irisCatalogRouter.get("/iris/catalog", requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    // The standard ten is a default preference only; it is not a ceiling on Iris intelligence.
    ...
    res.json({
      ...
      selection: { max: selectionLimit, count: selected.length, capability_ids: selected, selection_is_preference_not_ceiling: true },
      standard: { name: "Iris Standard", count: STANDARD_IDS.length, capability_ids: STANDARD_IDS, is_default_preference: true },
      catalog: CATALOG,
...
irisCatalogRouter.put("/iris/catalog/selection", requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    if (error) throw error;
    res.json({ saved: true, selection: { max: selectionLimit, count: ids.length, capability_ids: ids, selection_is_preference_not_ceiling: true } });
  } catch (error) {
