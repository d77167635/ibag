...
irisCatalogRouter.get("/iris/catalog", requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    const stored = hasStoredPreference && Array.isArray(data?.selected_capability_ids)
      ? [...new Set(data.selected_capability_ids.filter((id: unknown): id is string => typeof id === "string" && isKnown(id)))]
      : [];
    // A missing preference starts with the Iris Standard. A stored preference is an
    // unrestricted user selection: every catalog product can be independently active.
    const selected = hasStoredPreference ? stored : [...STANDARD_IDS];
    res.json({
      catalog_version: IRIS_CATALOG_VERSION,
      hierarchy: "Iris > products > analytical families > evidence-valid compositions > canonical evidence > Plaid source observations",
      ...
      selection: { max: CATALOG.length, count: selected.length, capability_ids: selected, selection_is_preference_not_ceiling: true, unlimited_within_catalog: true },
...
irisCatalogRouter.put("/iris/catalog/selection", requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    if (error) throw error;
    res.json({ saved: true, selection: { max: CATALOG.length, count: ids.length, capability_ids: ids, selection_is_preference_not_ceiling: true, unlimited_within_catalog: true } });
  } catch (error) {
