...
import { buildCompletePlaidFieldArchitecture } from "../intelligence/plaidFieldArchitecture.js";
...
plaidSurfaceRouter.get("/dashboard/plaid/surface", requireAuth, async (req: AuthedRequest, res) => {
  ...
  const completeFieldArchitecture = await buildCompletePlaidFieldArchitecture(req.userId!);
  res.json({ catalog_version: "2026-09-06-complete-provider-field-architecture-v4", source: "plaid_runtime_item_state_and_provider_domain_evidence", canonical_products: [...CANONICAL_PRODUCTS], items: itemSummaries, products, accounts: accounts ?? [], provider_evidence: rawEvidence, provider_evidence_counts: Object.fromEntries(CANONICAL_PRODUCTS.map((p) => [p, rawEvidence.filter((r: any) => r.product === p).length])), field_inventory: fieldInventory, evidence_field_inventory: evidenceFieldInventory, complete_field_architecture: completeFieldArchitecture, product_state_legend: { observed: "Direct live Plaid domain response recorded as current evidence.", active: "Plaid reports the product active but a current provider-domain observation is not recorded.", consented: "Plaid reports consent without current observed evidence.", available: "Plaid reports availability without current observed evidence.", not_available: "No current provider state for this canonical domain." }, source_boundary: "Plaid is the source/provider field architecture. The complete field universe is data-driven from current observed provider payloads and is not capped at eight products or twelve tiers. Supabase is persistence/evidence storage only." });
});
