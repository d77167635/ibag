...
export const PLAID_ITEM_PRODUCT_STATES = [
  ...
  "investments", "investments_auth", "liabilities", "payment_initiation", "identity_verification",
  "transactions", "credit_details", "income", "income_verification", "standing_orders", "transfer",
  "employment", "recurring_transactions", "transactions_refresh", "signal", "statements",
  "processor_payments", "processor_identity", "profile", "cra_base_report", "cra_income_insights",
  "cra_partner_insights", "cra_network_insights", "cra_cashflow_insights", "cra_monitoring",
  "cra_lend_score", "cra_plaid_credit_score", "cra_qualify", "cra_home_lending", "layer",
  "pay_by_bank", "protect_linked_bank", "protect_transactions",
] as const;
...
export function normalizePlaidItemProductStates(values: unknown): PlaidItemProductState[] {
  ...
  const result: PlaidItemProductState[] = [];
  for (const value of values) {
    if (typeof value === "string" && isPlaidItemProductState(value) && !result.includes(value)) result.push(value);
  }
  return result;
}
