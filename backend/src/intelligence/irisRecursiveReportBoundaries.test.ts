...
test("certification requires every independent boundary", () => {
  const evidence = evaluateReportEvidenceBoundary(["transactions"], [{ evidence_id: "obs-1", evidence_key: "transactions", state: "observed" }]);
  const certified = evaluateIrisReportCertificationGate({ executionStatus: "SUCCEEDED", runtimeLineage: lineage, semanticConsumption: semantic, evidenceBoundary: evidence });
  assert.equal(certified.certified, true);
  const failed = evaluateIrisReportCertificationGate({ executionStatus: "SUCCEEDED", runtimeLineage: { ...lineage, resolution_state: "partially_resolved" }, semanticConsumption: semantic, evidenceBoundary: evidence });
