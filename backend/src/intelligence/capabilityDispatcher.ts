...
import { getCapabilityOperator, type CapabilityOperatorResult, type GovernedCapabilityResult } from "./capabilityOperators.js";
...
export async function dispatchGovernedCapability({ userId, capabilityId }: DispatchRequest): Promise<CapabilityOperatorResult> {
  if (capabilityId === GOVERNED_AGGREGATE_CAPABILITY) {
    ...
    return { capability_id: GOVERNED_AGGREGATE_CAPABILITY, operator_id: GOVERNED_AGGREGATE_OPERATOR, operator_version: GOVERNED_AGGREGATE_OPERATOR_VERSION, evidence_state: "CALCULATED", result: result as GovernedCapabilityResult };
  }
