...
export function assessTrajectory(flows: WindowedFlow[]): { direction: "accelerating" | "decelerating" | "stable" | "insufficient_evidence"; shortWindowDailyRate: number | null; longWindowDailyRate: number | null } {
  ...
  const short = withData[0], long = withData[withData.length - 1];
  const shortRate = short.outflow / short.windowDays, longRate = long.outflow / long.windowDays;
  ...
  return { direction: deviation > 0.15 ? "accelerating" : deviation < -0.15 ? "decelerating" : "stable", shortWindowDailyRate: shortRate, longWindowDailyRate: longRate };
}
