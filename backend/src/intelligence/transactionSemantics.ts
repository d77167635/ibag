...
/** Window aggregation operates only on the supplied canonical transaction set. runId/evidenceBoundary are accepted to make the evidence-binding contract explicit; callers must supply transactions already resolved to that run. */
export function computeCanonicalWindowFlows(transactions:CanonicalTransaction[],windows:readonly number[],asOf?:string|Date|null,_runId?:string|null,evidenceBoundary?:string|Date|null){const boundary=boundaryDate(evidenceBoundary??asOf).getTime();return[...windows].sort((a,b)=>a-b).map(windowDays=>{const start=new Date(boundary-windowDays*86_400_000).toISOString().slice(0,10);const rows=transactions.filter(tx=>tx.posted_date>=start);const flow=computeEconomicCashFlow(rows);return{windowDays,...flow,purchaseTotal:rows.filter(tx=>tx.transaction_class==="purchase"&&tx.amount>0).reduce((s,tx)=>s+tx.amount,0),debtPaymentTotal:rows.filter(tx=>tx.transaction_class==="debt_payment"&&tx.amount>0).reduce((s,tx)=>s+tx.amount,0),txCount:rows.length,economicTxCount:rows.filter(tx=>isEconomicInflow(tx)||isEconomicOutflow(tx)).length};});}
