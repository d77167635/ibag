...
export type EmergentOperatorResult = {
  ...
  discoveries: Array<{
    discovery_type: "repeated_validated_outcome_type";
    discovery_key: string;
    source_fingerprints: string[];
    support_count: number;
    discovery: { repeated: true; outcome_type: string };
  }>;
  output_hash: string;
};

export async function executeEmergentOperator(userId: string): Promise<EmergentOperatorResult> {
  ...
  const payload = JSON.stringify({ userId, discoveries });
  return {
    capability_id: "emergent",
    operator_id: "emergent",
