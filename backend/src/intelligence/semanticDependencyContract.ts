...
export type SemanticDependencyContract = {
  ...
  /**
   * Contract-level semantic sufficiency rules. These do not prove mathematical truth;
   * they define the minimum executable semantic conditions under which Iris may treat
   * the capability output as sufficiently supported for its declared transformation.
   */
  sufficiency: {
    require_dependency_evidence: boolean;
    disallow_insufficient_dependency_evidence: boolean;
    disallow_insufficient_output_evidence: boolean;
  };
};

/**
 * Minimum dependency output paths that must be read at runtime.
 * Passing this is field-level execution evidence. Sufficiency is certified only when
...
export const SEMANTIC_DEPENDENCY_CONTRACTS: SemanticDependencyContract[] = [
  ...
  { capability_id: "relationship", requirements: [{ dependency_id: "pattern", required_paths: ["root.result.pattern_count"], rationale: "Relationship derivation must consume pattern output." }, { dependency_id: "relational_ontology", required_paths: ["root.result.relationships"], rationale: "Relationship derivation must consume relational ontology output." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "anomaly", requirements: [{ dependency_id: "analysis", required_paths: ["root.result"], rationale: "Anomaly derivation must consume analysis output." }, { dependency_id: "temporal", required_paths: ["root.result"], rationale: "Anomaly derivation must consume temporal context." }, { dependency_id: "behavioral", required_paths: ["root.result"], rationale: "Anomaly derivation must consume behavioral context." }, { dependency_id: "pattern", required_paths: ["root.result"], rationale: "Anomaly derivation must consume pattern context." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "causal", requirements: [{ dependency_id: "relationship", required_paths: ["root.result.relationships"], rationale: "Causal candidate analysis must consume relationship observations." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "predictive", requirements: [{ dependency_id: "causal", required_paths: ["root.result"], rationale: "Prediction must consume causal context." }, { dependency_id: "temporal", required_paths: ["root.result"], rationale: "Prediction must consume temporal context." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "scenario", requirements: [{ dependency_id: "predictive", required_paths: ["root.result.projection"], rationale: "Scenario composition must consume the predictive projection." }, { dependency_id: "risk", required_paths: ["root.result.risk_signals"], rationale: "Scenario composition must consume risk signals." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "decision", requirements: [{ dependency_id: "scenario", required_paths: ["root.result.scenarios"], rationale: "Decision intelligence must consume scenario output." }, { dependency_id: "risk", required_paths: ["root.result.risk_signals"], rationale: "Decision intelligence must consume risk output." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "recommendation", requirements: [{ dependency_id: "decision", required_paths: ["root.result.options"], rationale: "Recommendations must consume decision options." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "consequence", requirements: [{ dependency_id: "decision", required_paths: ["root.result"], rationale: "Consequence propagation must consume decision context." }, { dependency_id: "risk", required_paths: ["root.result.risk_signals"], rationale: "Consequence propagation must consume risk signals." }, { dependency_id: "opportunity", required_paths: ["root.result.opportunities"], rationale: "Consequence propagation must consume opportunity output." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "outcome", requirements: [{ dependency_id: "decision", required_paths: ["root.result"], rationale: "Outcome loop must consume decision context." }, { dependency_id: "recommendation", required_paths: ["root.result"], rationale: "Outcome loop must consume recommendation context." }, { dependency_id: "consequence", required_paths: ["root.result.consequences"], rationale: "Outcome loop must consume consequence output." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "learning", requirements: [{ dependency_id: "outcome", required_paths: ["root.result"], rationale: "Learning must consume outcome evidence." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
  { capability_id: "emergent", requirements: [{ dependency_id: "financial_life_state", required_paths: ["root.result.canonical_life_state"], rationale: "Higher-order synthesis must consume canonical life state." }, { dependency_id: "relational_ontology", required_paths: ["root.result.relationships"], rationale: "Higher-order synthesis must consume relational ontology." }], proof_required: true, sufficiency: { require_dependency_evidence: true, disallow_insufficient_dependency_evidence: true, disallow_insufficient_output_evidence: true } },
];
