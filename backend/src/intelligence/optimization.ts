...
export interface OptimizationScore { option_id: string; total_score: number; evidence_quality: number; expected_value: number; downside_exposure: number; reversibility: number; constraint_fit: number; objective_alignment: Record<OptimizationObjective, number>; explanation: string[]; }
export interface OptimizationIntelligence { architecture_version: "IRIS_OPTIMIZATION_INTELLIGENCE_V1"; objective: string; objectives: Array<{ objective: OptimizationObjective; priority: number; rationale: string }>; ranking_status: "ranked" | "constrained" | "blocked"; scores: OptimizationScore[]; preferred_option_id: string | null; tradeoff_summary: string[]; constraints: string[]; limitations: string[]; evidence: Evidence; }
const evidenceScore: Record<Evidence, number> = { observed: 1, calculated: 0.9, inferred: 0.65, insufficient_evidence: 0 };
const ALIGNMENT: Record<OptimizationObjective, Record<string, number>> = { stabilize_liquidity: { preserve_liquidity: 1, reduce_pressure: 0.9, investigate: 0.75, monitor: 0.7, optimize_roundups: 0.2 }, improve_cash_flow: { reduce_pressure: 1, preserve_liquidity: 0.85, investigate: 0.7, monitor: 0.65, optimize_roundups: 0.25 }, reduce_pressure: { reduce_pressure: 1, preserve_liquidity: 0.85, investigate: 0.7, monitor: 0.6, optimize_roundups: 0.3 }, build_roundups: { optimize_roundups: 1, monitor: 0.7, investigate: 0.55, preserve_liquidity: 0.3, reduce_pressure: 0.25 }, understand_finances: { investigate: 1, monitor: 0.9, preserve_liquidity: 0.55, reduce_pressure: 0.5, optimize_roundups: 0.45 } };
function consequenceFor(option: DecisionOption, model: ConsequenceModel): ConsequenceScenario | undefined { return model.scenarios.find((s) => s.decision_option_id === option.id); }
...
function deriveObjectives(state: FinancialStateModel, declaredGoals: DeclaredIrisGoal[]) {
  ...
  for (const goal of activeDeclared) if (!objectives.some((item) => item.objective === goal.objective)) objectives.push({ objective: goal.objective, priority: Math.max(1, goal.priority), rationale: `User-declared goal: ${goal.title}${goal.description ? ` — ${goal.description}` : ""}.` });
...
function hardConstraintApplies(constraint: string, option: DecisionOption): boolean {
  const normalized = constraint.trim().toLowerCase(); if (!normalized) return false;
...
function explicitConstraints(declaredGoals: DeclaredIrisGoal[]): string[] { return declaredGoals.filter((goal) => goal.active).flatMap((goal) => (goal.constraints ?? []).filter(Boolean).map((constraint) => `goal:${goal.id} — ${constraint}`)); }

/** Multi-objective, transparent analytical ranking. No execution or probabilistic outcome claim is made. */
export function buildOptimizationIntelligence(decision: DecisionIntelligence, consequences: ConsequenceModel, state: FinancialStateModel, declaredGoals: DeclaredIrisGoal[] = []): OptimizationIntelligence {
  ...
  if (!decision.options.length || decision.quality === "blocked") return { architecture_version: "IRIS_OPTIMIZATION_INTELLIGENCE_V1", objective: decision.objective, objectives, ranking_status: "blocked", scores: [], preferred_option_id: null, tradeoff_summary: ["No option can be responsibly ranked while decision evidence is blocked."], constraints, limitations, evidence: "insufficient_evidence" };
  let hardBlockedCount = 0;
  const scores = decision.options.map((option) => {
    const scenario = consequenceFor(option, consequences); const evidence = normalizeEvidence(scenario?.evidence ?? option.evidence_state); const evidenceQuality = evidenceScore[evidence];
    const reversibility = option.reversibility === "high" ? 1 : option.reversibility === "medium" ? 0.65 : 0.35;
    const downside = scenario?.tradeoffs.length ? Math.min(1, scenario.tradeoffs.length / 4) : 0; const benefit = scenario?.downstream_effects.length ? Math.min(1, scenario.downstream_effects.length / 4) : 0.25;
    const constraintPenalty = option.constraints.length ? Math.min(1, option.constraints.length / 5) : 0;
    const hardBlockedByGoal = declaredGoals.some((goal) => goal.active && (goal.constraints ?? []).some((constraint) => hardConstraintApplies(constraint, option))); if (hardBlockedByGoal) hardBlockedCount += 1;
    ...
    const weightFor = (priority: number) => priority === 1 ? 1 : priority === 2 ? 0.75 : priority === 3 ? 0.55 : 0.4; const totalWeight = objectives.reduce((sum, item) => sum + weightFor(item.priority), 0);
    const weightedAlignment = totalWeight ? objectives.reduce((sum, item) => sum + (objectiveAlignment[item.objective] ?? 0) * weightFor(item.priority), 0) / totalWeight : 0;
    const expectedValue = benefit * 0.3 + weightedAlignment * 0.15; const total = hardBlockedByGoal ? 0 : Math.max(0, Math.min(1, evidenceQuality * 0.3 + expectedValue + reversibility * 0.2 - downside * 0.1 - constraintPenalty * 0.1 - liquidityPenalty));
    const explanation = [`Evidence quality contributes ${evidenceQuality.toFixed(2)} to the ranking.`, `Goal-weighted multi-objective alignment contributes ${weightedAlignment.toFixed(2)} across ${objectives.length} objectives.`, `Reversibility is ${option.reversibility} and contributes ${reversibility.toFixed(2)}.`, `Modeled benefit signal is ${benefit.toFixed(2)}; downside exposure signal is ${downside.toFixed(2)}.`];
    ...
    if (hardBlockedByGoal) explanation.push("Blocked by an explicit user-declared constraint; no analytical preference can override that boundary."); if (liquidityPenalty) explanation.push("Liquidity pressure reduces the analytical preference for increasing Round-Up exposure."); if (option.constraints.length) explanation.push("Option-specific constraints reduce its score.");
  ...
  const preferred = scores.find((score) => score.total_score > 0) ?? null; const second = preferred ? scores.find((score) => score.option_id !== preferred.option_id && score.total_score > 0) : undefined;
  ...
  summary.push("Objective priority follows user-declared goals where present; financial safety states remain governing constraints."); if (userConstraints.length) summary.push(`Explicit user constraints are active: ${userConstraints.length} ${userConstraints.length === 1 ? "boundary" : "boundaries"}.`);
