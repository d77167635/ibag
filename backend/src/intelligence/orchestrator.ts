...
import { evaluateIntelligenceValidation } from "./validationEngine.js";
import { buildProviderDomainIntelligence } from "./providerDomainIntelligence.js";
import { buildIrisGovernorAssembly } from "./irisGovernor.js";
import { buildIrisFeatureRegistry } from "./irisFeatureRegistry.js";
import { buildPlaidIrisFieldMap } from "./plaidIrisFieldMap.js";
import { buildIrisFieldArchitecture } from "./irisFieldArchitecture.js";
import { buildPlaidProductComposition } from "./plaidProductComposition.js";

/** Canonical intelligence orchestrator for Dashboard and Iris. */
export async function computeFullIntelligence(userId: string) {
  const [evidenceBoundary, sourceFidelity, providerDomainIntelligence, plaidIrisFieldMap] = await Promise.all([getCertifiedEvidenceBoundary(userId), assessSourceFidelity(userId), buildProviderDomainIntelligence(userId), buildPlaidIrisFieldMap(userId)]);
  ...
  const irisGovernor = buildIrisGovernorAssembly({ evidenceGate: { status: sourceFidelity.status, ready: sourceFidelity.ready_for_higher_order_intelligence }, plaidProducts: Object.keys(providerDomainIntelligence.domains ?? {}), layers: intelligenceAtlas.definitions.map(d => d.id), intelligenceOutputs: ["narrative", "financial_state", "cash_flow", "spending", "behavior", "reasoning", "decisions", "simulation", "maximum_intelligence", "higher_order_synthesis", "adversarial_reasoning", "counterfactual_intelligence", "meta_intelligence"], compositionOutputs: ["intelligence_composition", "layer_composition", "cross_domain_findings"], validationOutputs: ["integrity", "source_fidelity", "uncertainty", "provider_lineage"], composition: layerComposition, validation });
  const featureRegistry = buildIrisFeatureRegistry(layerComposition.new_analysis_features, Object.keys(providerDomainIntelligence.domains ?? {}));
  ...
  const intelligenceForFieldArchitecture = { ...baseResult, integrity, source_fidelity: sourceFidelity, intelligence_gate: intelligenceGate, evidence_graph: evidenceGraph, intelligence_graph: intelligenceGraph, investigations, uncertainty, financial_state: financialState, causal_analysis: causalAnalysis, decision_graph: decisionGraph, decision_intelligence: decisionIntelligence, consequence_model: consequenceModel, optimization_intelligence: optimization, goal_intelligence: goals, intelligence_atlas: intelligenceAtlas, intelligence_composition: composition, layer_composition: layerComposition, higher_order_synthesis: higherOrderSynthesis, adversarial_reasoning: adversarialReasoning, counterfactual_intelligence: counterfactualIntelligence, meta_intelligence: metaIntelligence, validation, iris_governor: irisGovernor, iris_feature_registry: featureRegistry };
  const irisFieldArchitecture = buildIrisFieldArchitecture(userId, intelligenceForFieldArchitecture, plaidIrisFieldMap.provider_field_universe);
  const plaidProductComposition = buildPlaidProductComposition(Object.keys(providerDomainIntelligence.domains ?? {}));
  recordExplainabilityTrace(userId, reasoning).catch(err => console.error("explainability trace failed:", err));
  recordIntelligenceSnapshot(userId, { evidenceBoundary, liquidAssets: balances.liquidAssets, cashFlowNet: cashFlow.net, safeToSpend: cashFlowSafety.safeToSpend, revolvingDebt: balances.revolvingDebt, creditUtilization: balances.creditUtilization, forwardProjectedLiquidPosition: typeof forwardProjection.projectedLiquidPosition === "number" ? forwardProjection.projectedLiquidPosition : null, roundupProjected: roundupProjection.projectedAmount ?? roundupProjection.projectedTotal ?? null, sourceFidelityStatus: sourceFidelity.status ?? null, higherOrderReady: sourceFidelity.ready_for_higher_order_intelligence, forecastHorizonDays: 30, metadata: { atlas_ready: intelligenceAtlas.counts.evidence_ready, atlas_total: intelligenceAtlas.counts.total_defined, investigation_count: investigations.investigations.length, composition_possible: rawComposition.counts.possible_combinations, layer_composition_ready: layerComposition.counts.evidence_ready, new_analysis_features: layerComposition.counts.new_analysis_features, feature_registry_candidates: featureRegistry.features.length, provider_domain_evidence_ready: providerDomainIntelligence.evidence_ready, provider_domain_selected_item: providerDomainIntelligence.selected_item_id, plaid_field_map_data_records: plaidIrisFieldMap.counts.data_records, plaid_field_map_unique_fields: plaidIrisFieldMap.counts.unique_fields, plaid_complete_source_fields: plaidIrisFieldMap.provider_field_universe?.counts?.unique_provider_fields ?? null, iris_derived_fields: irisFieldArchitecture.derived_field_count, plaid_product_combinations: plaidProductComposition.counts.non_empty_combinations, plaid_product_sequences: plaidProductComposition.counts.ordered_sequences } }).catch(err => console.error("intelligence snapshot failed:", err));
  return { ...intelligenceForFieldArchitecture, iris_field_architecture: irisFieldArchitecture, plaid_product_composition: plaidProductComposition };
}
