...
export async function computeFullIntelligence(userId: string) {
  ...
  const adversarialReasoning = buildAdversarialReasoning({ causalAnalysis, findings: higherOrderSynthesis.findings, investigations: investigations.investigations });
  const counterfactualIntelligence = buildCounterfactualIntelligence({ decision: decisionIntelligence, optimization: optimizationBase, safeToSpend: cashFlowSafety.safeToSpend, cashFlowNet: cashFlow.net, revolvingDebt: balances.revolvingDebt });
