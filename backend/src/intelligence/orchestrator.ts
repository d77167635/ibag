...
import { buildAdversarialReasoning } from "./adversarialReasoning.js";
...
export async function computeFullIntelligence(userId: string) {
  // Iris must establish the evidence boundary and authorize execution before
  // any downstream intelligence executor is invoked. Provider evidence is
  // never modified by this gate.
  const [evidenceBoundary, sourceFidelity] = await Promise.all([getCertifiedEvidenceBoundary(userId), assessSourceFidelity(userId)]);
  const irisPreflight = buildIrisPreflight(sourceFidelity, evidenceBoundary);
  if (!irisPreflight.execute_downstream_intelligence) {
    throw new Error(`IRIS_PRE_EXECUTION_BLOCKED: ${irisPreflight.reasons.join("; ") || "Evidence execution gate is closed."}`);
  }

  const providerDomainIntelligence = await buildProviderDomainIntelligence(userId);
  ...
  const layerComposition = buildLayerCompositionEngine(intelligenceAtlas);
  ...
  const irisGovernance = buildIrisGovernance({ providerDomainIntelligence, sourceFidelity, integrity, evidenceGraph, intelligenceGraph, investigations, composition, layerComposition, higherOrderSynthesis, adversarialReasoning, counterfactualIntelligence, metaIntelligence, atlasDefinitions: intelligenceAtlas.definitions });
  ...
  recordIntelligenceSnapshot(userId, { evidenceBoundary, liquidAssets: balances.liquidAssets, cashFlowNet: cashFlow.net, safeToSpend: cashFlowSafety.safeToSpend, revolvingDebt: balances.revolvingDebt, creditUtilization: balances.creditUtilization, forwardProjectedLiquidPosition: typeof forwardProjection.projectedLiquidPosition === "number" ? forwardProjection.projectedLiquidPosition : null, roundupProjected: roundupProjection.projectedAmount ?? roundupProjection.projectedTotal ?? null, sourceFidelityStatus: sourceFidelity.status ?? null, higherOrderReady: sourceFidelity.ready_for_higher_order_intelligence, forecastHorizonDays: 30, metadata: { iris_preflight_status: irisPreflight.status, iris_preflight_selected_item: irisPreflight.selected_item_id, atlas_ready: intelligenceAtlas.counts.evidence_ready, atlas_total: intelligenceAtlas.counts.total_defined, investigation_count: investigations.investigations.length, composition_possible: rawComposition.counts.possible_combinations, layer_composition_ready: layerComposition.counts.evidence_ready, new_analysis_features: layerComposition.counts.new_analysis_features, provider_domain_evidence_ready: providerDomainIntelligence.evidence_ready, provider_domain_selected_item: providerDomainIntelligence.selected_item_id, iris_governance_status: irisGovernance.status, iris_governance_alert_count: irisGovernance.alerts.length } }).catch(err => console.error("intelligence snapshot failed:", err));
  return { ...baseResult, integrity, iris_preflight: irisPreflight, source_fidelity: sourceFidelity, intelligence_gate: intelligenceGate, evidence_graph: evidenceGraph, intelligence_graph: intelligenceGraph, investigations, uncertainty, financial_state: financialState, causal_analysis: causalAnalysis, decision_graph: decisionGraph, decision_intelligence: decisionIntelligence, consequence_model: consequenceModel, optimization_intelligence: optimization, goal_intelligence: goals, intelligence_atlas: intelligenceAtlas, intelligence_composition: composition, layer_composition: layerComposition, higher_order_synthesis: higherOrderSynthesis, adversarial_reasoning: adversarialReasoning, counterfactual_intelligence: counterfactualIntelligence, meta_intelligence: metaIntelligence, iris_governance: irisGovernance };
}
