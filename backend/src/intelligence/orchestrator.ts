...
import type { CapabilityExecutionContext } from "./capabilityOperators.js";
...
export async function computeFullIntelligence(userId: string, context?: CapabilityExecutionContext) {
  ...
  const current30Start = effectiveBoundary ? new Date(new Date(effectiveBoundary).getTime() - 30 * 86_400_000).toISOString().slice(0, 10) : new Date(Date.now() - 30 * 86_400_000).toISOString().slice(0, 10);
  ...
  const spendingByDomain = computeSpendingByDomainFromTransactions(canonical, 30, effectiveBoundary);
  const spendingHierarchy = computeCanonicalSpendingHierarchy(canonical, 30, effectiveBoundary);
  ...
  const [balances, cashFlowSafety, balanceHistory, debtTrend, anomalies, forwardProjection, debtCost, categoryDrift, multiWindowFlow, reasoning, featureFlags, declaredGoalsResult, providerLineage] = await Promise.all([
    ...
    computeFinancialReasoning(userId, effectiveBoundary),
