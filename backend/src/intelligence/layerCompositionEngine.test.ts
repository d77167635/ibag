...
test("recursive layer composition materializes pathways deeper than two layers", () => {
  ...
  const result = buildLayerCompositionEngine({ definitions }, { maxDepth: 4, maxCandidates: 100, beamWidth: 12 });
...
test("evidence-limited recursive pathways remain explicitly limited", () => {
  ...
  const result = buildLayerCompositionEngine({ definitions }, { maxDepth: 3, maxCandidates: 50, beamWidth: 8 });
