...
import type { IrisExecutionRecord, IrisExecutionInput, IrisExecutionOutput, IrisValidationState } from "./irisExecutionTypes.js";
...
export async function updateExecutionValidation(userId: string, executionId: string, status: IrisValidationState): Promise<IrisExecutionRecord> {
  const { data, error } = await supabaseAdmin.from("iris_execution_records").update({ validation_status: status }).eq("id", executionId).eq("user_id", userId).select("*").single();
  if (error) throw error;
  return data as IrisExecutionRecord;
}
