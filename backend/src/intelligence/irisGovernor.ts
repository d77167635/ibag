...
export type IrisGovernorAssembly = {
  architecture_version: "IRIS_GOVERNOR_V5";
  iris: {
    available_on_every_screen: true;
    supreme_intelligence: true;
    governs_all_intelligence: true;
    customer_facing_governance_controls: false;
  };
  evidence_gate: { status: string; ready: boolean; financial_facts_created: false };
  data_plane: {
    connection: "Plaid Link";
    ingestion: "Plaid authorized data";
  ...
  capabilities: {
    ...
    cross_domain_composition: boolean;
  ...
  governance: {
    ...
    validated_requires_governed_promotion: true;
    customer_admin_role_required: false;
    lineage_required: true;
    double_counting_prohibited: true;
    cycles_prohibited: true;
    evidence_gate_precedes_promotion: true;
...
export function evaluateIrisGovernorOperation(
  input: Omit<IrisGovernorOperation, "status" | "financial_values_created" | "provider_observations_created" | "execution_capability">,
): IrisGovernorOperation {
  ...
  return {
    ...
    provider_observations_created: false,
    execution_capability: false,
    limitation: evidenceReady
      ? "Candidate analytics require validation and governed promotion; Iris never silently changes production intelligence."
      : "Operation cannot be evaluated until its evidence boundary, inputs, and ordered operation path are complete.",
  };
}

/**
 * Final read-only assembly point. Runtime materialization is bounded for safety,
 * but intelligence depth is not conceptually capped. Iris governs selection,
 * composition, validation, explanation, monitoring, learning, and revalidation.
 */
export function buildIrisGovernorAssembly(input: {
  ...
  return {
    ...
    inputs: { data_domains: uniq(input.dataDomains), intelligence_layers: uniq(input.layers), intelligence_outputs: uniq(input.intelligenceOutputs), composition_outputs: uniq(input.compositionOutputs), validation_outputs: uniq(input.validationOutputs) },
    composition_engine: {
    ...
    capabilities: {
      arbitrary_ordered_composition: true,
      cross_domain_composition: input.dataDomains.length > 1,
