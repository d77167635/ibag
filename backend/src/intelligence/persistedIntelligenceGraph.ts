...
export const PERSISTED_INTELLIGENCE_GRAPH_VERSION = "iris-persisted-intelligence-graph-v6" as const;
...
/**
 * Persist a recursively derived intelligence node. Arbitrary higher-order
 * compositions must have explicit upstream intelligence nodes; only governed
 * capability-root nodes may be persisted without an intelligence-node parent.
 */
export async function persistArbitraryDerivedIntelligenceNode(input: { userId: string; runId: string; executionId: string; definition: ArbitraryDerivedIntelligenceDefinition; allowRoot?: boolean }): Promise<PersistedNode> {
  ...
  if (!upstream.length && !input.allowRoot) throw new Error("DERIVED_INTELLIGENCE_UPSTREAM_REQUIRED");
