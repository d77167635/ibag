...
export async function buildPlaidIrisFieldMap(userId: string) {
  ...
  const authorityResult = await supabaseAdmin.from("plaid_product_observations")
  ...
  const authorityByProduct = Object.fromEntries(canonicalProducts.map((product) => [product,
    authority.filter((row: any) => row.product === product).map((row: any) => ({
      id: row.id, item_id: row.item_id, lifecycle_state: row.lifecycle_state, evidence_state: row.evidence_state,
      observed_at: row.observed_at, acquired_at: row.acquired_at, is_current: row.is_current, provider_object_id: row.provider_object_id,
    }))
  ]));
  return {
    ...
    structural_depth: architecture.structural_depth,
    counts: {
      ...architecture.counts,
    ...
    rules: [
      ...
      "Products and capabilities are applications over the field and intelligence universe, not architecture tiers.",
    ],
  };
}
