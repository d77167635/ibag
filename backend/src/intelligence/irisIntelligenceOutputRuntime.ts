...
export interface IrisIntelligenceOutputRuntime {
  ...
  publication_policy: { ready: string; limited: string; suppressed: string; report_activation_is_user_control: true; catalog_metadata_alone_is_not_evidence: true; feature_to_analysis_mapping_is_one_to_many: true; headline_binding_is_conservative: true; runtime_lineage_required_for_certified_publication: true };
...
function headline(definition: Pick<AtlasDefinition, "id" | "name">, state: IrisIntelligenceOutputState, evidenceCoverage: number, evidencePublicationState: IrisEvidencePublicationState) { return selectHeadlineIntelligence({ analysis_id: definition.id, analysis_name: definition.name, state, evidence_coverage: evidenceCoverage, evidence_publication_state: evidencePublicationState }); }

export function buildIrisIntelligenceOutputRuntime(atlas: { definitions: AtlasDefinition[] }, featureRuntime: IrisFeatureRuntimeSnapshot, activeReportIds: string[] = [], runtimeLineage: Record<string, IrisReportRuntimeLineage> = {}): IrisIntelligenceOutputRuntime {
  ...
  const definitions: IrisIntelligenceOutput[] = atlas.definitions.flatMap((definition): IrisIntelligenceOutput[] => {
    ...
    if (state.readiness === "ready" && definition.evidence_ready === true && lineage?.resolution_state === "resolved") return features.map((feature) => ({ ...base("ready", "calculated", coverage, "atlas_readiness", [], [], null), feature_id: feature.featureId, capability_id: feature.capabilityId }));
