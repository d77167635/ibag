...
function addMetricNode(nodes: EvidenceNode[], id: string, label: string, value: unknown, state: EvidenceState, source: string, providerDomain?: string) {
  ...
  if (["cash_flow_net", "safe_to_spend", "roundup_projection", "category_drift"].includes(id)) node.observation_window_days = 30;
  if (["forward_projection", "trajectory", "anomalies"].includes(id)) node.observation_window_days = 90;
  nodes.push(node);
}
...
export function buildEvidenceGraph(intel: any): EvidenceGraph {
  ...
  const add = (id: string, label: string, value: unknown, state: EvidenceState, source: string, providerDomain?: string) => addMetricNode(nodes, id, label, value, state, source, providerDomain);
  const has = (id: string) => nodes.some((node) => node.id === id);
  const link = (from: string, to: string, relation: EvidenceRelation) => {
    if (has(from) && has(to) && !edges.some((edge) => edge.from === from && edge.to === to && edge.relation === relation)) edges.push({ from, to, relation });
  };
  ...
  if (metrics.debt_health?.revolving_debt != null) add("revolving_debt", "Revolving debt", metrics.debt_health.revolving_debt, "observed", "Plaid credit account balances", "balance");
  if (metrics.debt_health?.credit_utilization != null) add("credit_utilization", "Credit utilization", metrics.debt_health.credit_utilization, "calculated", "Iris credit utilization calculation", "balance");
  ...
  if (metrics.roundup_projection) {
    const value = metrics.roundup_projection;
    add("roundup_projection", "Round-Up projection", value.projectedAmount ?? value.projected, "calculated", "Iris canonical Round-Up calculation", "transactions");
    const created = nodes.find((n) => n.id === "roundup_projection");
    if (created && value.calculation_version) created.calculation_version = String(value.calculation_version);
    if (created && value.basisDays != null) created.observation_window_days = Number(value.basisDays);
  }
  ...
  link("revolving_debt", "debt_cost", "supports");
  link("credit_utilization", "debt_cost", "supports");
  ...
  link("revolving_debt", "safe_to_spend", "constrains");
  ...
  for (const node of nodes) node.upstream_node_ids = edges.filter((edge) => edge.to === node.id).map((edge) => edge.from);
