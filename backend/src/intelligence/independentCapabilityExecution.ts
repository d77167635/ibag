...
async function persistRunEvidence(runId: string, userId: string, selectedItemId: string | null) {
  let evidenceQuery = supabaseAdmin
    ...
    .select("id,item_id,product,raw_response,effective_at,acquired_at")
    .eq("user_id", userId)
    .eq("is_current", true)
  ...
  if (evidenceReadError) {
    throw new Error(`RUN_EVIDENCE_READ_FAILED: ${evidenceReadError.message}`);
  }
  ...
  if (existingError) {
    throw new Error(`RUN_EVIDENCE_EXISTING_READ_FAILED: ${existingError.message}`);
  }

  const existingIds = new Set(
    (existingEvidence ?? [])
      .map((row) => row.raw_observation_id)
  ...
  const rows = rawEvidence
    ...
    .map((e) => ({
      ...
      provider: "plaid",
      product: e.product,
      raw_observation_id: e.id,
      effective_at: e.effective_at ?? e.acquired_at,
      acquired_at: e.acquired_at,
      evidence_hash: hash(e.raw_response),
    }));

  if (!rows.length) return existingIds.size;

  const { error: evidenceInsertError } = await supabaseAdmin
    .from("iris_run_evidence")
    .insert(rows);

  if (evidenceInsertError) {
    throw new Error(`RUN_EVIDENCE_PERSIST_FAILED: ${evidenceInsertError.message}`);
  }
...
export async function executeIndependentCapabilities(request: {
  ...
  if (existing?.id) {
    ...
    return {
      ...existing,
      execution_ids: results.map((x) => x.execution.id),
      results,
      certified: existing.status === "CERTIFIED",
    };
  }

  const selectedItemId = await resolveCanonicalProviderItem(userId);
  const evidenceScope: IrisEvidenceScope = {
    kind: selectedItemId ? "provider_item" : "user_aggregate",
    selectedItemId,
    canonicalProviderDomains: IRIS_CANONICAL_PROVIDER_DOMAINS,
  };

  const plan = await planCapabilities(userId, requestedCapabilities);
  if (plan.status === "BLOCKED") {
    throw new Error(`CAPABILITY_PLAN_BLOCKED: ${plan.limitations.join(" | ")}`);
  }
  ...
  const { data: run, error: runError } = await supabaseAdmin
    ...
    .insert({
      ...
      execution_policy: {
        ...
        certification_profile: "independent_capability",
      },
      planner_version: PLANNER_VERSION,
      orchestrator_version: ORCHESTRATOR_VERSION,
      certification_policy_version: CERTIFICATION_POLICY_VERSION,
  ...
  if (runError || !run) {
    throw new Error(`Unable to create Iris run: ${runError?.message || "unknown error"}`);
  }
  ...
  try {
    ...
    for (const capabilityId of executionOrder) {
      ...
      if (!operator?.execute || operator.status !== "implemented") {
      ...
      const { data: execution, error: executionError } = await supabaseAdmin
        ...
        .insert({
          ...
          started_at: executionStarted,
          evidence_state: "CALCULATED",
          validation_status: "UNKNOWN",
          certification_status: "PENDING",
          input_manifest: executionManifest,
        })
        .select("*")
        .single();

      if (executionError || !execution) {
        throw new Error(`EXECUTION_RECORD_CREATE_FAILED: ${executionError?.message || "unknown error"}`);
      }
      ...
      try {
        ...
        const { error: outputError } = await supabaseAdmin
          ...
          .insert({
            ...
            value: result,
            hash: outputHash,
            evidence_state: dispatched.evidence_state,
        ...
        const { error: executionUpdateError } = await supabaseAdmin
          .from("iris_execution_records")
          .update({
            execution_state: "EXECUTED",
            completed_at: finishedAt,
            input_hash: executionInputHash,
            ...
            resource_usage: { duration_ms: durationMs },
          })
          .eq("id", execution.id)
          .eq("user_id", userId);

        if (executionUpdateError) {
          throw new Error(`EXECUTION_RECORD_FINALIZE_FAILED: ${executionUpdateError.message}`);
        }

        const gate = await evaluateIndependentCapabilityCertification({
          runId: run.id,
        ...
        const validationRows = Object.entries(gate.checks).map(([ruleId, gateCheck]) => ({
          run_id: run.id,
          execution_id: execution.id,
          user_id: userId,
          rule_id: ruleId,
          ...
          severity: gateCheck.status === "PASS" ? "INFO" : "CRITICAL",
          expected: { status: "PASS" },
          actual: { status: gateCheck.status },
          details: {
            message: gateCheck.details,
            gate_version: CERTIFICATION_POLICY_VERSION,
            capability_id: capabilityId,
        ...
        if (gate.eligible) {
          const certificationHash = hash({
            ...
            capability_id: capabilityId,
            contract_version: contract.version,
            evidence: gate.evidence_snapshot,
          });
          ...
          const { error: certificationError } = await supabaseAdmin.rpc("finalize_iris_capability_certification", {
            ...
            p_user_id: userId,
            p_policy_version: CERTIFICATION_POLICY_VERSION,
            p_validation_snapshot: { status: "PASS", checks: gate.checks, contract_version: contract.version },
            p_reconciliation_snapshot: gate.reconciliation_snapshot,
            p_evidence_snapshot: gate.evidence_snapshot,
            p_certification_hash: certificationHash,
            p_certified_at: new Date().toISOString(),
          });

          if (certificationError) {
            throw new Error(`CERTIFICATION_FINALIZE_FAILED: ${certificationError.message}`);
          }
          ...
          allCertified = false;
          const { error: validationUpdateError } = await supabaseAdmin
            .from("iris_execution_records")
            .update({ validation_status: "FAIL", certification_status: "NOT_CERTIFIED" })
            .eq("id", execution.id)
            .eq("user_id", userId);

          if (validationUpdateError) {
            throw new Error(`CERTIFICATION_STATUS_UPDATE_FAILED: ${validationUpdateError.message}`);
          }
        ...
        await supabaseAdmin
          ...
          .update({
            ...
            error_code: "INTELLIGENCE_EXECUTION_FAILED",
    ...
    return {
      ...
      id: run.id,
      status: finalStatus,
      execution_ids: results.map((x) => x.execution_id),
      results,
    ...
    await supabaseAdmin
      ...
      .update({
        status: "FAILED",
        failure_code: "INTELLIGENCE_EXECUTION_FAILED",
        failure_message: errorText(error),
        completed_at: now,
        updated_at: now,
      })
      .eq("id", run.id)
