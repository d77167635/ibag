...
export async function executeIndependentCapabilities(request: { userId: string; requestId?: string; surface?: string; mode?: string; requestedCapabilities: string[] }) {
  const userId = request.userId; const requestId = request.requestId?.trim() || randomUUID(); const surface = request.surface || "iris"; const mode = request.mode || "capability"; const requestedCapabilities = [...new Set(request.requestedCapabilities.filter(Boolean))]; const asOf = new Date().toISOString();
  ...
  const plan = await planCapabilities(userId, requestedCapabilities); if (plan.status === "BLOCKED") throw new Error(`CAPABILITY_PLAN_BLOCKED: ${plan.limitations.join(" | ")}`);
  ...
  const results: Array<Record<string, unknown>> = []; let allCertified = true;
  try {
    const executionOrder = plan.execution_order.filter((id: string) => requestedCapabilities.includes(id));
    ...
    for (const capabilityId of executionOrder) {
      ...
      const executionManifest = { ...initialManifest, capability_id: capabilityId, operator_id: operator.operator_id, operator_version: operator.version };
      ...
      try {
        const { error: inputError } = await supabaseAdmin.from("iris_execution_inputs").insert({ execution_id: execution.id, input_type: "execution_manifest", reference_type: "iris_run", reference_id: run.id, role: "primary", hash: executionInputHash });
        if (inputError) throw new Error(`EXECUTION_INPUT_PERSIST_FAILED: ${inputError.message}`);
        const dispatched = await dispatchGovernedCapability({ userId, capabilityId }); const result = dispatched.result; const outputHash = hash(result); const finishedAt = new Date().toISOString();
        const { error: outputError } = await supabaseAdmin.from("iris_execution_outputs").insert({ execution_id: execution.id, output_key: capabilityId, output_type: "intelligence_capability", value: result, hash: outputHash, evidence_state: dispatched.evidence_state, uncertainty: result.uncertainty ?? null });
        if (outputError) throw new Error(`EXECUTION_OUTPUT_PERSIST_FAILED: ${outputError.message}`);
        let evidenceQuery = supabaseAdmin.from("plaid_raw_product_observations").select("id,item_id,product,raw_response,effective_at,acquired_at").eq("user_id", userId).eq("is_current", true).eq("evidence_state", "observed"); if (selectedItemId) evidenceQuery = evidenceQuery.eq("item_id", selectedItemId);
        const rawEvidence = (await evidenceQuery).data ?? [];
        if (rawEvidence.length) { const { error: evidenceInsertError } = await supabaseAdmin.from("iris_run_evidence").insert(rawEvidence.map(e => ({ run_id: run.id, user_id: userId, evidence_type: "provider_raw_observation", provider: "plaid", product: e.product, raw_observation_id: e.id, effective_at: e.effective_at ?? e.acquired_at, acquired_at: e.acquired_at, evidence_hash: hash(e.raw_response) }))); if (evidenceInsertError) throw new Error(`RUN_EVIDENCE_PERSIST_FAILED: ${evidenceInsertError.message}`); }
        await supabaseAdmin.from("iris_runs").update({ status: "EXECUTED", updated_at: finishedAt }).eq("id", run.id).eq("user_id", userId);
        const durationMs = Date.parse(finishedAt) - Date.parse(executionStarted);
        await supabaseAdmin.from("iris_execution_records").update({ execution_state: "EXECUTED", completed_at: finishedAt, input_hash: executionInputHash, output_hash: outputHash, output_snapshot: { output_key: capabilityId, output_hash: outputHash, evidence_scope: evidenceScope, dispatched_capability: dispatched.capability_id, dispatched_operator: dispatched.operator_id, dispatched_operator_version: dispatched.operator_version }, resource_usage: { duration_ms: durationMs }, validation_status: "UNKNOWN" }).eq("id", execution.id).eq("user_id", userId);
        const gate = await evaluateIndependentCapabilityCertification({ runId: run.id, executionId: execution.id, userId, inputHash: executionInputHash, outputHash });
        ...
        const { error: validationError } = await supabaseAdmin.from("iris_validation_results").insert(validationRows); if (validationError) throw new Error(`VALIDATION_PERSIST_FAILED: ${validationError.message}`);
        if (gate.eligible) {
          const certificationHash = hash({ run_id: run.id, execution_id: execution.id, input_hash: executionInputHash, output_hash: outputHash, policy: CERTIFICATION_POLICY_VERSION, capability_id: capabilityId, evidence: gate.evidence_snapshot });
          const { error: certificationError } = await supabaseAdmin.from("iris_certifications").insert({ run_id: run.id, execution_id: execution.id, user_id: userId, result_id: execution.id, policy_version: CERTIFICATION_POLICY_VERSION, status: "CERTIFIED", validation_snapshot: { status: "PASS", checks: gate.checks }, reconciliation_snapshot: gate.reconciliation_snapshot, evidence_snapshot: gate.evidence_snapshot, certification_hash: certificationHash, certified_at: new Date().toISOString() }); if (certificationError) throw new Error(`CERTIFICATION_PERSIST_FAILED: ${certificationError.message}`);
          await supabaseAdmin.from("iris_execution_records").update({ validation_status: "PASS", certification_status: "CERTIFIED" }).eq("id", execution.id).eq("user_id", userId); results.push({ capability_id: capabilityId, execution_id: execution.id, result, certified: true, certification_gate: gate });
        } else { allCertified = false; await supabaseAdmin.from("iris_execution_records").update({ validation_status: "FAIL", certification_status: "NOT_CERTIFIED" }).eq("id", execution.id).eq("user_id", userId); results.push({ capability_id: capabilityId, execution_id: execution.id, result, certified: false, certification_gate: gate }); }
      } catch (error) { allCertified = false; const now = new Date().toISOString(); await supabaseAdmin.from("iris_execution_records").update({ execution_state: "FAILED", completed_at: now, validation_status: "FAIL", certification_status: "NOT_CERTIFIED", error_code: "INTELLIGENCE_EXECUTION_FAILED", error_message: errorText(error) }).eq("id", execution.id).eq("user_id", userId); throw error; }
    }
    const finalStatus = allCertified ? "CERTIFIED" : "VALIDATION_FAILED"; const now = new Date().toISOString(); await supabaseAdmin.from("iris_runs").update({ status: finalStatus, completed_at: now, updated_at: now }).eq("id", run.id).eq("user_id", userId); return { ...run, id: run.id, status: finalStatus, execution_ids: results.map(x => x.execution_id), results, certified: allCertified };
  } catch (error) { const now = new Date().toISOString(); await supabaseAdmin.from("iris_runs").update({ status: "FAILED", failure_code: "INTELLIGENCE_EXECUTION_FAILED", failure_message: errorText(error), completed_at: now, updated_at: now }).eq("id", run.id).eq("user_id", userId); throw error; }
}
