...
export type IntelligenceValidation = {
  architecture_version: "IRIS_VALIDATION_ENGINE_V4";
  sample_count: number;
...
function horizonFrom(row: any): number | null {
  const value = Number(row?.metadata?.forecast_horizon_days);
  return Number.isFinite(value) && value > 0 ? value : null;
}

/**
 * Validates forecasts only against a later real snapshot that is at least the
 * forecast horizon away. Missing forecast-horizon metadata is excluded rather
 * than silently assigned a default, because a validation horizon is part of
 * the prediction contract and must itself be evidenced by the snapshot.
 */
export async function evaluateIntelligenceValidation(userId: string): Promise<IntelligenceValidation> {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    ...
    observations.push({
      ...
      horizon_days: Math.round(horizonDays),
  ...
  return {
    ...
    horizon_days: horizonValues.length === 1 ? horizonValues[0] : null,
    observations: observations.slice(-24),
    limitations: [
      "Validation requires later real evidence snapshots; no future outcome is invented.",
      "Predictions without explicit forecast-horizon metadata are excluded rather than assigned an assumed horizon.",
