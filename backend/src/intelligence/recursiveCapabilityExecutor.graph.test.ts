...
test("recursive executor returns the exact persisted graph-node ID for every executed capability", async () => {
  const persisted: Record<string, string> = {};
  const execution = await executeRecursiveCapabilityPlan("user-test", plan(), { runId: "run-test", executionId: "execution-test", persistSemanticDependencyProof: async () => {}, persistSemanticTransformationEdges: async () => {}, persistGraphNode: async ({ capabilityId, dependencyNodeIds }) => { if (capabilityId === "child") assert.deepEqual(dependencyNodeIds, { parent: "node-parent" }); const id = `node-${capabilityId}`; persisted[capabilityId] = id; return { id }; } }, { maxNodes: 10, maxEdges: 10, maxCompositions: 10 }, async ({ capabilityId, context }) => { if (capabilityId === "parent") return result("parent", { value: 1 }); assert.equal(context?.dependencyResults?.parent?.result.value, 1); return result("child", { value: 2 }); });
  assert.equal(execution.status, "COMPLETED"); assert.deepEqual(execution.graph_node_ids, persisted); assert.deepEqual(execution.graph_node_ids, { parent: "node-parent", child: "node-child" });
});
