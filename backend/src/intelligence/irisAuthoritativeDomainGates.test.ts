...
function buildSnapshot(brokenDomain: string | null = null) {
  ...
  const sourceFields: FixtureRow[] = IRIS_AUTHORITATIVE_DOMAINS.map((domain, index) => ({ id: `fixture:source-field:${index}`, family_node_id: `fixture:family:${index}`, product: domain.product, active: true }));
  const observations: FixtureRow[] = IRIS_AUTHORITATIVE_DOMAINS.map((domain) => ({ id: `fixture:observation:${domain.domain}`, product: domain.product, evidence_state: "observed", raw_observation_id: `fixture:raw:${domain.domain}`, user_id: USER_ID }));
  ...
  for (const domain of IRIS_AUTHORITATIVE_DOMAINS) {
    ...
    for (const [capability, evidenceId, nodeId] of [["temporal", temporalEvidence, temporalNode], ["analysis", analysisEvidence, analysisNode], ["behavioral", behavioralEvidence, behavioralNode]] as const) {
      ...
      lineage.push({ user_id: USER_ID, run_id: RUN_ID, execution_id: EXECUTION_ID, lineage_role: "SOURCE_EVIDENCE", source_type: "run_evidence", source_id: evidenceId, destination_type: "intelligence_node", destination_id: nodeId, evidence_state: "observed" });
      runtimeNodes.push({ user_id: USER_ID, run_id: RUN_ID, execution_id: EXECUTION_ID, id: nodeId, capability_id: capability, intelligence_key: capability, evidence_state: "CALCULATED", recursive_ancestry: capability === "temporal" ? [] : capability === "analysis" ? [temporalNode] : [temporalNode, analysisNode], upstream_node_ids: capability === "temporal" ? [] : capability === "analysis" ? [temporalNode] : [analysisNode] });
    }
