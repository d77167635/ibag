...
const DEFAULT_BUDGET: IrisResourceBudget = {
  ...
  max_provider_acquisitions: 16,
  max_concurrent_runs: 1,
};
const POLICY: IrisExecutionPolicy = {
  version: "IRIS_EXECUTION_POLICY_V1",
  certification_policy_version: "IRIS_CERTIFICATION_POLICY_V1",
...
export async function executeIrisFullIntelligenceRun(input: {
  ...
  const run = await createRun({
    ...
    request_surface: input.requestSurface ?? "dashboard",
    request_mode: input.requestMode ?? "full_intelligence",
    requested_capabilities: input.requestedCapabilities ?? ["iris.full_intelligence"],
    status: "PLANNED",
    as_of: asOf,
    evidence_boundary: evidenceBoundary,
    evidence_version: manifest.hash,
    resource_budget: DEFAULT_BUDGET,
    execution_policy: POLICY,
    planner_version: "IRIS_PLANNER_V1",
    orchestrator_version: "ORCHESTRATOR_COMPAT_737CEF",
    certification_policy_version: POLICY.certification_policy_version,
    financial_context_hash: null,
  ...
  try {
    ...
    const execution = await startExecution({
      run_id: run.id,
      user_id: input.userId,
      capability_id: "iris.full_intelligence",
      operator_id: "computeFullIntelligence",
      operator_version: "ORCHESTRATOR_COMPAT_737CEF",
      execution_state: "EXECUTING",
      input_hash: manifest.hash,
      output_hash: null,
      evidence_state: manifest.references.length ? "OBSERVED" : "INSUFFICIENT_EVIDENCE",
      input_manifest: { evidence_manifest_hash: manifest.hash, evidence_count: manifest.references.length },
