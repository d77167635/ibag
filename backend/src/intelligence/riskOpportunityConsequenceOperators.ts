...
function wrap(
  capabilityId: string,
  output: GovernedCapabilityResult,
  ...
  return {
    ...
    result: {
      ...
      evidence: {
        ...
        transaction_count: output.transaction_count ?? null,
      },
      provenance: {
        source: "certified_capability_outputs",
...
function available(context: CapabilityExecutionContext | undefined, ids: string[]): boolean {
  return ids.some(id => dep(context, id)?.evidence_state !== "INSUFFICIENT_EVIDENCE");
}

function anomalyCount(value: GovernedCapabilityResult | null): number {
  const n = value?.detection_count;
  return typeof n === "number" && Number.isFinite(n) ? n : Array.isArray(value?.detections) ? value.detections.length : 0;
}

function outflow(value: GovernedCapabilityResult | null): number | null {
  const flow = value?.flow;
  if (!flow || typeof flow !== "object") return null;
  const n = (flow as Record<string, unknown>).outflow;
...
export async function executeRisk(userId: string, context?: CapabilityExecutionContext): Promise<CapabilityOperatorResult> {
  ...
  if (anomaly && anomalyCount(anomaly) > 0) {
    signals.push({ type: "observed_anomaly_signal", count: anomalyCount(anomaly), severity: "review", basis: "robust anomaly detections" });
  }
  ...
  if (projection && typeof projection === "object") {
    ...
    if (typeof p.ending_balance === "number" && p.ending_balance < 0) {
      signals.push({ type: "projected_negative_balance_signal", severity: "high", basis: "constrained forward projection", projected_ending_balance: p.ending_balance });
    }
  }

  const evidenceAvailable = available(context, ["analysis", "behavioral", "anomaly", "predictive"]);
  const state = evidenceAvailable ? "INFERRED" : "INSUFFICIENT_EVIDENCE";
  return wrap("risk", {
    risk_signals: signals,
    signal_count: signals.length,
    risk_state: signals.length ? "signals_present" : evidenceAvailable ? "no_material_signal_detected" : "insufficient_evidence",
    observed_outflow: outflow(analysis),
...
export async function executeOpportunity(userId: string, context?: CapabilityExecutionContext): Promise<CapabilityOperatorResult> {
  ...
  if (Array.isArray(spending)) {
    for (const item of spending.slice(0, 10)) {
  ...
  if (behavioral?.merchant_frequency && Array.isArray(behavioral.merchant_frequency)) {
    const concentrated = behavioral.merchant_frequency.slice(0, 5).filter((x: unknown) => x && typeof x === "object");
    for (const merchant of concentrated) opportunities.push({ type: "merchant_review", basis: "observed frequency", merchant });
  }
...
export async function executeConsequence(userId: string, context?: CapabilityExecutionContext): Promise<CapabilityOperatorResult> {
  ...
  if (risk?.risk_signals && Array.isArray(risk.risk_signals)) {
  ...
  if (opportunity?.opportunities && Array.isArray(opportunity.opportunities)) {
