...
const REQUIRED: Record<IrisProductIntent, string[][]> = {
  ...
  liquidity: [["balance"], ["transactions"]], debt: [["liabilities"], ["balance"], ["transactions"]], roundups: [["transactions"]], anomaly: [["transactions"]],
...
const ACTUAL_CONSUMPTION: Record<IrisProductIntent, Record<string, string[]>> = {
  overview: { transactions: ["cash_flow", "spending"], balance: ["liquidity"] }, cash_flow: { transactions: ["cash_flow"] }, spending: { transactions: ["spending"] },
  liquidity: { transactions: ["cash_flow"], balance: ["liquidity"] }, debt: { liabilities: ["debt_cost"], balance: ["debt_health"], transactions: ["debt_trend"] },
  roundups: { transactions: ["roundups"] }, anomaly: { transactions: ["anomalies"] }, explanation: {}, provider_data: {}, unknown: {},
};
...
export function chooseIrisCombinations(observedByItem: ObservedByItem, intent: IrisProductIntent) {
  ...
  const required = requiredGroups.map((group) => ({ acceptable_products: group, satisfied_on_item: [...observedByItem.entries()].filter(([, products]) => group.some((p) => products.has(p))).map(([item_id]) => item_id) }));
  const optional = (OPTIONAL[intent] ?? []).map((group) => ({ acceptable_products: group, satisfied_on_item: [...observedByItem.entries()].filter(([, products]) => group.some((p) => products.has(p))).map(([item_id]) => item_id) }));
  const requiredItemIds = [...observedByItem.entries()].filter(([, products]) => requiredGroups.every((group) => group.some((p) => products.has(p)))).map(([item_id]) => item_id);
  const candidates = COMBINATION_LIBRARY.map((combo) => {
    const matchingItems = [...observedByItem.entries()].filter(([, products]) => combo.products.every((product) => products.has(product))).map(([item_id]) => item_id);
    const globallyObserved = new Set([...observedByItem.values()].flatMap((products) => [...products]));
    const missing_products = combo.products.filter((product) => !globallyObserved.has(product));
    const cross_item_conflict_products = combo.products.filter((product) => globallyObserved.has(product) && !matchingItems.length);
    return { ...combo, evidence_ready: matchingItems.length > 0, matching_item_ids: matchingItems, missing_products, cross_item_conflict_products };
  });
  return {
    intent, required, optional, evidence_ready: requiredGroups.length === 0 || requiredItemIds.length > 0, required_item_ids: requiredItemIds,
    ready_combinations: candidates.filter((c) => c.evidence_ready),
    blocked_combinations: candidates.filter((c) => !c.evidence_ready).map(({ key, missing_products, cross_item_conflict_products }) => ({ key, missing_products, cross_item_conflict_products })),
