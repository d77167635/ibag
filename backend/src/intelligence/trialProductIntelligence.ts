...
// Each inner group is an OR group. Multiple groups are ANDed. This prevents
// the prior bug where [balance, transactions] meant either product was enough.
...
function evaluateGroups(groups: string[][], observed: Set<string>) {
  return groups.map((group) => ({ acceptable_products: group, observed_products: group.filter((product) => observed.has(product)), satisfied: group.some((product) => observed.has(product)) }));
}

function chooseCombinations(observedByItem: Map<string, Set<string>>, intent: IrisProductIntent) {
  const required = (REQUIRED[intent] ?? []).map((group) => ({ acceptable_products: group, satisfied_on_item: [...observedByItem.entries()].filter(([, products]) => group.some((p) => products.has(p))).map(([item_id]) => item_id) }));
  ...
  const candidates = COMBINATION_LIBRARY.map((combo) => {
    ...
    return { ...combo, evidence_ready: matchingItems.length > 0, matching_item_ids: matchingItems, missing_products: combo.products.filter((product) => ![...observedByItem.values()].some((products) => products.has(product))) };
  });
  return {
    intent,
    required,
    optional,
    evidence_ready: required.every((r) => r.satisfied_on_item.length > 0),
...
export async function buildTrialProductIntelligence(userId: string, intent: IrisProductIntent = "unknown") {
  ...
  // Authority alone is never sufficient. The product must have a matching,
  // current, observed provider-source mirror on the same Plaid Item.
  ...
  return {
    ...
    selection: chooseCombinations(observedByItem, intent),
