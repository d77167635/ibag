 ...
 for(const[product,analysisKeys]of Object.entries(actualConsumption)){
   ...
   for(const authority of authoritiesForProduct){
     ...
     for(const analysisKey of analysisKeys){
       consumptionRows.push({user_id:userId,item_id:authority.item_id,product,analysis_key:analysisKey,evidence_observation_id:authority.id,raw_observation_id:authoritativeSpecialized?(specialized[0]?.id??null):(rawProduct?.id??null),details:{evidence_state:"observed",acquired_at:authoritativeSpecialized?(specialized.map(r=>r.acquired_at).sort().at(-1)??authority.acquired_at):(rawProduct?.acquired_at??authority.acquired_at),source_kind:sourceKind,source_observation_ids:sourceIds,intent,combination_keys:combinationKeys,selected_item_id:selection.selected_item_id,consumption:"request_path",authoritative_source:true}});
     }
   }
 }
