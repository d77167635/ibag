...
function numberAt(value: any, fields: string[]): number | null { for (const field of fields) { const n = Number(value?.[field]); if (Number.isFinite(n)) return n; } return null; }
function sumNumbers(rows: any[], fields: string[]): number | null { const values = rows.map(row => numberAt(row, fields)).filter((n): n is number => n !== null); return values.length ? values.reduce((a, b) => a + b, 0) : null; }
function uniqueById(rows: any[]): any[] { const seen = new Set<string>(); return rows.filter(row => { const key = String(row?.statement_id ?? row?.id ?? row?.statement_date ?? JSON.stringify(row)); if (seen.has(key)) return false; seen.add(key); return true; }); }
function currencyOf(row: any): string | null { const value = row?.raw_response?.balances?.iso_currency_code ?? row?.raw_response?.iso_currency_code ?? row?.iso_currency_code; return typeof value === "string" && value.trim() ? value.trim().toUpperCase() : null; }
function sameCurrency(rows: RawCore[]): { currency: string | null; safe: boolean } { const currencies = new Set(rows.map(currencyOf).filter((v): v is string => Boolean(v))); if (currencies.size === 1) return { currency: [...currencies][0], safe: true }; return { currency: null, safe: false }; }

/** Provider monetary values remain in Plaid currency units at this boundary. No database money columns are mutated. */
export async function buildProviderDomainIntelligence(userId: string) {
  ...
  if (authorityError) throw authorityError; if (rawError) throw rawError; if (accountError) throw accountError; if (txError) throw txError; if (balanceError) throw balanceError; if (liabilityError) throw liabilityError;
  ...
  const txRows = attach(rawTransactions ?? []), balanceRows = attach(rawBalances ?? []), liabilityRows = attach(rawLiabilities ?? []);
  const raw = (rawProducts ?? []) as RawProduct[], auth = (authorities ?? []) as Authority[];
  const byItem = new Map<string, Set<string>>();
  const add = (itemId: string, product: string) => { const set = byItem.get(itemId) ?? new Set<string>(); set.add(product); byItem.set(itemId, set); };
  for (const a of auth) add(a.item_id, a.product); for (const row of txRows) add(row.item_id, "transactions"); for (const row of balanceRows) add(row.item_id, "balance"); for (const row of liabilityRows) add(row.item_id, "liabilities");
  ...
  const source = (product: Product) => {
    ...
    return { authority, specialized, generic, ids: authoritativeSpecialized ? specialized.map(r => r.id) : generic.map(r => r.id), authoritativeSpecialized };
  };
  const authSource = source("auth"), identitySource = source("identity"), assetSource = source("assets"), investmentSource = source("investments"), statementSource = source("statements"), liabilitySource = source("liabilities"), transactionSource = source("transactions"), balanceSource = source("balance");
  ...
  const netWorth = balanceCurrency.safe && (liquidBalance !== null || investmentAccountBalance !== null || debtAccountBalance !== null) ? (liquidBalance ?? 0) + (investmentAccountBalance ?? 0) - (debtAccountBalance ?? 0) : null;
  ...
  for (const plan of consumptionPlans) {
    const s = source(plan.product), authority = s.authority;
