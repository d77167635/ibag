export type MetaIntelligence = {
  ...
  quality: { evidence_coverage: number; analytical_coverage: number; higher_order_readiness: boolean; bottleneck_count: number; label: "strong" | "moderate" | "limited" | "insufficient" };
  bottlenecks: Array<{ input: string; analyses_blocked: number; analysis_ids: string[]; leverage: "very_high" | "high" | "medium" | "low"; statement: string }>;
...
export function buildMetaIntelligence(input: Input): MetaIntelligence {
  ...
  return {
    architecture_version: "IRIS_META_INTELLIGENCE_V1",
    quality: { evidence_coverage: Math.round(evidenceCoverage * 1000) / 1000, analytical_coverage: Math.round(analyticalCoverage * 1000) / 1000, higher_order_readiness: higherOrderReady, bottleneck_count: bottlenecks.length, label: label(score) },
    bottlenecks, highest_leverage_next_steps: nextSteps,
    integrity_signals: [
