...
export function reconcileFinancialComposition(
  ...
  const liquidAssets = currencySafe ? sum(depository) : null;
  const investmentAccounts = currencySafe ? sum(investments) : null;
  const debtAccounts = currencySafe ? sum(debt) : null;
  const netWorth = currencySafe && accounts.length > 0 ? (liquidAssets ?? 0) + (investmentAccounts ?? 0) - (debtAccounts ?? 0) : null;
  ...
  const failed = duplicateAccountIds.length > 0;
