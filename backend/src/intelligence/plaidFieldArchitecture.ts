...
function stableId(parts: string[]) {
  return `plaid-field-${createHash("sha256").update(parts.join("|"), "utf8").digest("hex").slice(0, 24)}`;
}

function safeValue(key: string, value: unknown) {
  return PROTECTED_KEYS.test(key) ? { protected: true, value: null } : { protected: false, value };
}
...
function flatten(value: unknown, path: string, source: SourceRecord, out: any[]) {
  if (value === null || typeof value !== "object") {
    ...
    out.push({
      field_id: stableId([source.source_table, source.id, path]),
      ...
      path,
      field_name: name,
      exact_provider_field_name: name,
      value: safe.value,
      protected: safe.protected,
      value_type: value === null ? "null" : typeof value,
      evidence_state: "observed",
      acquired_at: source.acquired_at,
      provider_object_id: source.provider_object_id ?? null,
      lineage: `plaid:${source.product}:${source.id}:${path}`,
    });
    return;
  }
  if (Array.isArray(value)) {
    value.forEach((child, index) => flatten(child, `${path}[${index}]`, source, out));
    return;
  }
  Object.keys(value as Record<string, unknown>).sort().forEach((key) => {
    flatten((value as Record<string, unknown>)[key], path ? `${path}.${key}` : key, source, out);
  });
}
...
function addHierarchy(root: any, field: any) {
  ...
  segments.forEach((segment: string, index: number) => {
    ...
    if (!child) {
      child = { name: clean, depth: index + 1, children: [], field_count: 0 };
...
/**
 * Complete provider-field architecture. The hierarchy is data-driven: it is not
 * capped at 8 products or 12 tiers. Product/capability names come from current
 * provider observations; field names and values come from the provider payload.
 */
export async function buildCompletePlaidFieldArchitecture(userId: string) {
  ...
  const fields: any[] = [];
  for (const record of records) if (record.raw_response !== null && record.raw_response !== undefined) flatten(record.raw_response, "$", record, fields);
  ...
  const byProduct = Object.fromEntries(products.map((product) => {
    const productFields = [...unique.values()].filter((f) => f.product === product);
    return [product, { field_count: productFields.length, record_count: records.filter((r) => r.product === product).length, fields: productFields }];
  }));

  const relationalAccountFields = (accounts.data ?? []).flatMap((account: any) => Object.entries(account).filter(([key]) => !PROTECTED_KEYS.test(key)).map(([key, value]) => ({
    field_id: stableId(["plaid_accounts", account.id, key]), source_table: "plaid_accounts", product: "account", source_record_id: account.id, item_id: account.item_id, account_id: account.id, path: `$${key}`, field_name: key, exact_provider_field_name: key, value, protected: false, value_type: value === null ? "null" : typeof value, evidence_state: "observed", lineage: `plaid:account:${account.id}:${key}`
  })));

  const maxDepth = [...unique.values()].reduce((m, f) => Math.max(m, f.canonical_path.split(".").length), 0);
  const tierCount = Math.max(1, maxDepth + 2);
  const tierDefinitions = Array.from({ length: tierCount }, (_, index) => ({ tier: index + 1, meaning: index === 0 ? "provider" : index === 1 ? "product_or_capability" : index === tierCount - 1 ? "field_value" : `provider_payload_hierarchy_level_${index + 1}`, derived_from_actual_field_structure: true }));

  return {
    architecture_version: "PLAID_COMPLETE_SOURCE_FIELD_ARCHITECTURE_V1",
    source: "plaid",
    source_boundary: "provider observations only; Supabase is persistence and evidence storage, not an Iris intelligence source",
    exact_provider_field_names: true,
    exact_provider_values: true,
    fabricated_values: false,
    tier_policy: "dynamic; tier count is determined by observed provider payload structure and is never capped at 8 or 12",
    tier_definitions: tierDefinitions,
    products_and_capabilities: products,
    records,
    fields: [...unique.values(), ...relationalAccountFields],
    provider_payload_fields: [...unique.values()],
    relational_account_fields: relationalAccountFields,
    by_product: byProduct,
    hierarchy,
    counts: { records: records.length, provider_payload_field_occurrences: fields.length, unique_provider_fields: unique.size, relational_account_fields: relationalAccountFields.length, total_fields: unique.size + relationalAccountFields.length, products_or_capabilities: products.length, max_payload_depth: maxDepth },
    rules: [
      "Provider field names are preserved; Iris does not rename provider fields.",
      "Provider payload values are copied exactly when not protected secrets.",
      "Protected credentials remain represented as protected fields but their values are never exposed.",
      "Only current observed provider evidence is included in the live field universe.",
      "Array indexes are normalized only for canonical_path grouping; original path remains available on each occurrence.",
      "Architecture depth follows the actual payload; there is no arbitrary tier ceiling."
    ]
  };
}
