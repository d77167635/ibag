...
export async function computeFinancialReasoning(userId: string, asOf?: string | null, executionContext?: IrisExecutionContext): Promise<FinancialReasoning> {
  const widest = 90;
  ...
  return { risks, opportunities, relationalChain:chain, unresolvedQuestions:unresolved, priorityFocus:priorityFocus ? {key:priorityFocus.key,reason:priorityFocus.statement}:null, generatedAt:executionContext?.temporal.as_of ?? new Date().toISOString() };
}
