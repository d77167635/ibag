...
export async function computeFinancialReasoning(userId: string): Promise<FinancialReasoning> {
  ...
  const currentCutoff = new Date(Date.now() - 30 * 86_400_000).toISOString().slice(0, 10);
  ...
  const [balances, cashSafety, debtTrend, anomalies, debtCost, drift, multiWindow] = await Promise.all([
    computeBalanceMetrics(userId), computeCashFlowSafety(userId), computeDebtTrend(userId), detectAnomalies(userId),
    computeDebtCostIntelligence(userId), computeCategoryDrift(userId), computeMultiWindowFlow(userId),
  ]);
  ...
  if (prior.length) {
    const priorFlow = computeEconomicCashFlow(prior);
    if (priorFlow.net !== 0) cashFlow.netChangePct = ((canonicalCashFlow.net - priorFlow.net) / Math.abs(priorFlow.net)) * 100;
  }
  ...
  if (cashFlow.net !== null && balances.liquidAssets !== null) chain.push(
    cashFlow.net < 0
      ? `Economic cash flow over the last 30 days is negative (${cashFlow.net.toFixed(2)}), alongside liquid assets of ${balances.liquidAssets.toFixed(2)}.`
      : `Economic cash flow over the last 30 days is positive (${cashFlow.net.toFixed(2)}), alongside liquid assets of ${balances.liquidAssets.toFixed(2)}.`
  );
  ...
  else unresolved.push(debtCost.basis);
  if (trajectory.direction === "accelerating") chain.push("Economic spending pace is accelerating relative to the longer-window baseline.");
  else if (trajectory.direction === "insufficient_evidence") unresolved.push("Not enough economic transaction history across multiple time windows to assess spending pace.");
