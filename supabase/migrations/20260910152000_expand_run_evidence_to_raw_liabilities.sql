-- Extend exact run evidence to provider liability observations.
-- Liability intelligence must never fall back to unbound current liability rows
-- when a run-bound execution is being certified.
...
begin
  ...
  select evidence_boundary into run_boundary
  ...
  where t.user_id=new.user_id and a.item_id=obs_item and t.is_current=true and t.evidence_state='observed'
    and t.acquired_at <= coalesce(run_boundary,new.acquired_at)
  ...
  where b.user_id=new.user_id and a.item_id=obs_item and b.is_current=true and b.evidence_state='observed'
    and b.acquired_at <= coalesce(run_boundary,new.acquired_at)
  ...
  insert into public.iris_run_evidence
    (run_id,user_id,evidence_type,provider,product,raw_observation_id,effective_at,acquired_at,evidence_hash)
  select new.run_id,new.user_id,'provider_raw_liability','plaid','liabilities',l.id,l.effective_at,l.acquired_at,coalesce(l.observation_hash, md5(l.raw_response::text))
  from public.plaid_raw_liabilities l
  join public.plaid_accounts a on a.id=l.account_id and a.user_id=l.user_id
  where l.user_id=new.user_id and a.item_id=obs_item and l.is_current=true and l.evidence_state='observed'
    and l.acquired_at <= coalesce(run_boundary,new.acquired_at)
    and not exists (select 1 from public.iris_run_evidence e where e.run_id=new.run_id and e.raw_observation_id=l.id and e.evidence_type='provider_raw_liability');
