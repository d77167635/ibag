...
BEGIN
  ...
  END IF;
  SELECT * INTO current_row FROM public.plaid_product_observations
  WHERE user_id=p_user_id AND item_id=p_item_id AND provider='plaid' AND product=p_product AND is_current
  FOR UPDATE;
  ...
  SELECT COALESCE(MAX(observation_version),0)+1 INTO next_version FROM public.plaid_product_observations
  ...
  INSERT INTO public.plaid_product_observations (
    ...
    now(),p_provenance,p_product,now(),now(),p_evidence_state,next_version,current_row.id,true,
